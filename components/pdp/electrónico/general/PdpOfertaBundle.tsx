...
import { TrustBar, InlineCODForm, StickyBuyButton, FAQSection, FeatureGrid, GuaranteeBadge, ComparisonTable, HowItWorks } from '@/components/pdp/SharedCRO';
...
export default function PdpOfertaBundle({ data, product, variant = 1 }: PDPProps) {
  ...
  const originalBasePrice = Number(product.originalPrice) || 0;
        ...
        <div className={`grid ${layout.bundleStyle === 'grid' ? 'grid-cols-2' : 'grid-cols-1 sm:grid-cols-2'} gap-4`}>
          {bundles.map((bundle) => (
            ...
            >
              ...
              <div className={`font-black text-2xl ${isDark || variant === 9 ? 'text-white' : 'text-slate-800'}`}>{fmtPrice(bundle.price)}</div>
              {bundle.save > 0 && (
                <div className="mt-2 flex flex-col items-center">
                  <span className={`text-xs ${isDark || variant === 9 ? 'text-zinc-400' : 'text-slate-400'} line-through`}>{fmtPrice(bundle.oldPrice)}</span>
          ...
          {bundles.map((bundle) => (
            ...
            >
              ...
              <div className="text-right">
                <div className="font-black text-2xl text-black">{fmtPrice(bundle.price)}</div>
              </div>
            </div>
          ))}
        </div>
            ...
            <div className="text-right">
              ...
              {bundle.save > 0 && (
                <div className="flex items-center justify-end space-x-2">
                  <span className={`text-xs ${isDark || variant === 9 ? 'text-zinc-500' : 'text-slate-400'} line-through`}>{fmtPrice(bundle.oldPrice)}</span>
    ...
    <div className={`min-h-screen ${isDark ? 'bg-zinc-950 text-white' : 'bg-slate-50 text-slate-900'} font-sans pb-24`}>
      ...
      <main className={layout.main}>
        <div className={layout.grid}>
          ...
          <div className={layout.media}>
            ...
            <div className={`relative overflow-hidden ${variant === 5 ? 'border-4 border-black' : 'rounded-2xl shadow-lg border-4'} ${isDark ? 'border-zinc-800' : 'border-white'}`}>
              ...
              <EnhancedProductGallery 
                product={product}
                accentColor={theme.primary.replace('bg-', '#')}
              />
            </div>
          ...
          <div className={layout.info}>
            ...
            <p className={`${isDark || variant === 9 ? 'text-zinc-400' : 'text-slate-600'} text-base leading-relaxed mb-8 font-medium ${variant === 2 ? 'text-center' : ''}`}>
              {ai?.enhancedDescription || product.description} Aprovecha nuestros descuentos por volumen antes de que se agoten.
            </p>
            ...
            <div className={`${variant === 5 ? 'bg-black text-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(255,0,0,1)]' : isDark || variant === 9 ? 'bg-zinc-800' : 'bg-slate-900'} text-white p-6 rounded-2xl shadow-xl mb-8 relative overflow-hidden`}>
              ...
              <div className="relative z-10">
                <div className="flex justify-between items-end mb-2">
                  ...
                  <div className="text-right">
                    <span className={`text-sm ${isDark || variant === 9 ? 'text-zinc-400' : 'text-slate-400'} line-through mr-2`}>{fmtPrice(selected.oldPrice)}</span>
                    <span className="text-4xl font-black">{fmtPrice(selected.price)}</span>
                  </div>
                </div>
                {selected.save > 0 && product.discount && product.discount > 0 && (
                  <div className={`${isDark || variant === 9 ? 'text-zinc-300' : 'text-emerald-400'} text-sm font-bold text-right`}>
                    ¡Estás ahorrando {product.discount}% hoy!
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="mt-12 space-y-12">
          <FeatureGrid iconColor={theme.text} bgClass={theme.bgLight} />
