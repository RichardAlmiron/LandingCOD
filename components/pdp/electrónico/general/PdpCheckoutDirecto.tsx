...
import { TrustBar, InlineCODForm, ScarcityWarning, FeatureGrid, GuaranteeBadge, ComparisonTable, HowItWorks } from '@/components/pdp/SharedCRO';
...
export default function PdpCheckoutDirecto({ data, product, variant = 1 }: PDPProps) {
          ...
          <div className={`${layout.media} ${isDark && variant !== 9 ? 'bg-zinc-900 border-zinc-800' : variant !== 5 && variant !== 8 && variant !== 9 && variant !== 2 && variant !== 3 && variant !== 4 && variant !== 6 && variant !== 7 && variant !== 10 ? 'bg-zinc-50/50 border-zinc-100' : ''}`}>
            <div className={variant === 10 ? 'md:w-1/2' : ''}>
              ...
              <div className={`flex items-end space-x-3 mb-6 ${variant === 2 ? 'justify-center' : ''}`}>
                <span className={`text-5xl font-black ${isDark || variant === 8 ? 'text-white' : 'text-zinc-900'} tracking-tighter`}>{fmtPrice(product.price)}</span>
                <span className={`text-xl ${isDark || variant === 8 ? 'text-zinc-500' : 'text-zinc-400'} line-through font-bold mb-1`}>{fmtPrice(product.originalPrice)}</span>
              </div>
              ...
              <div className={`relative overflow-hidden shadow-md mb-6 ${layout.imageStyle} ${variant === 5 ? '' : isDark || variant === 8 ? 'border-zinc-800 border' : 'border-zinc-200 border'}`}>
                ...
                />
                <div className={`absolute bottom-4 right-4 ${isDark || variant === 8 ? 'bg-zinc-900/90 text-white' : 'bg-white/90 text-zinc-900'} backdrop-blur-sm px-3 py-1.5 rounded-full text-sm font-bold shadow-sm flex items-center space-x-1 z-10`}>
            ...
            <form className="space-y-5 mt-6" onSubmit={(e) => e.preventDefault()}>
              ...
              <div className={`${isDark || variant === 6 || variant === 9 ? 'bg-zinc-900 border-zinc-800' : 'bg-zinc-50 border-zinc-200'} p-4 ${variant === 5 ? 'rounded-none border-4 border-black' : 'rounded-xl border'} mt-6 mb-6`}>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className={`font-bold ${isDark || variant === 6 || variant === 9 ? 'text-white' : 'text-zinc-900'}`}>{fmtPrice(product.price)}</span>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-2xl font-black ${isDark || variant === 6 || variant === 9 ? 'text-white' : 'text-zinc-900'}`}>{fmtPrice(product.price)}</span>
                </div>
              </div>
