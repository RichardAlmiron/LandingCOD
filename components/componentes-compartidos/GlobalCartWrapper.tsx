...
import { useFloatingCart, FloatingCartProvider } from '@/contexts/FloatingCartContext';
...
interface GlobalCartWrapperProps {
  data: StoreData;
  children: React.ReactNode;
}

// Inner component that uses the cart context
function CartComponents({ data }: { data: StoreData }) {
  const {
    cartItems,
    updateQuantity,
    removeFromCart,
  ...
  const hasWhatsAppNumber = data.floatingCartConfig?.whatsappNumber && 
    data.floatingCartConfig.whatsappNumber.split('|')[1]?.length > 0;
  const floatingCartEnabled = data.floatingCartConfig?.enabled && hasWhatsAppNumber;
  ...
  const handleOrderSubmit = (formData: {
    ...
    const pcMessage = 
      `*NUEVO PEDIDO - ${data.name.toUpperCase()}*\n` +
      `═══════════════════════════════════\n` +
    ...
    const whatsappNumber = data.floatingCartConfig?.whatsappNumber || data.whatsappNumber || '595981123456';
...
// Wrapper component - proporciona su propio contexto para funcionar independientemente
export default function GlobalCartWrapper({ data, children }: GlobalCartWrapperProps) {
  return (
