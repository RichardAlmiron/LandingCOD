    ...
    return (
      ...
      >
        ...
        <div className="flex items-center gap-2 mt-1">
          <span className="font-bold text-sm">{product.currency || 'S/'}{product.price}</span>
          {product.originalPrice && (
            <span className="text-xs text-gray-500 line-through">{product.currency || 'S/'}{product.originalPrice}</span>
          )}
        </div>
      </div>
    );
    ...
    return (
      ...
      >
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white/90 px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10">
          ...
          <span className="text-xs font-bold text-red-600">{product.currency || 'S/'}{product.price}</span>
        </div>
      </div>
    );
  }
      ...
      <div className="group flex gap-4 p-4 border border-gray-200 rounded-lg hover:shadow-md transition-shadow">
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex items-center gap-2 mt-2">
            <span className="font-bold text-xl">{product.currency || 'S/'}{product.price.toFixed(2)}</span>
            {product.originalPrice && (
              <span className="text-gray-500 line-through">{product.currency || 'S/'}{product.originalPrice.toFixed(2)}</span>
            )}
          </div>
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-2">
          <span className="font-bold text-lg">{product.currency || 'S/'}{product.price}</span>
          {product.originalPrice && (
            <span className="text-sm text-gray-500 line-through">{product.currency || 'S/'}{product.originalPrice}</span>
          )}
        </div>
      </div>
    </div>
