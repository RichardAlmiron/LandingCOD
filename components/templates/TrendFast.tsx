              ...
              <div key={`flash-${i}`} className="group cursor-pointer relative flex flex-col">
                ...
                <div className="flex-1 flex flex-col px-1">
                  <div className="flex items-baseline gap-1.5 mb-[2px]">
                    ...
                    <span className="text-[12px] text-gray-400 line-through">${product.originalPrice || (parseFloat(product.price) * 1.5).toFixed(2)}</span>
                  </div>

                  {/* Progress bar */}
                  <div className="w-full bg-[#f2f2f2] h-[5px] rounded-full mt-1 overflow-hidden relative mb-1">
                    <div className="bg-[#fa6338] h-full rounded-full" style={{ width: `${Math.floor(Math.random() * 60) + 30}%` }}></div>
                  </div>
                  <div className="text-[10px] text-gray-500 uppercase tracking-widest font-semibold mb-2">{Math.floor(Math.random() * 50) + 10} Vendidos</div>
              ...
              <div key={`trendfast-${idx}-${product.id}`} data-product-id={product.id} className="group cursor-pointer flex flex-col">
                ...
                {Math.random() > 0.4 && (
                  <div className="flex items-center gap-1 text-[11px] text-gray-500 px-1 mt-auto">
                    ...
                    <span>({Math.floor(Math.random() * 900) + 10} Revisiones)</span>
                  </div>
                )}
              </div>
            ))}
