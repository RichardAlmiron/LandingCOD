...
import { TrustBar, InlineCODForm, StickyBuyButton, FAQSection, RecentSalesPopup, FeatureGrid, GuaranteeBadge, ComparisonTable, HowItWorks } from './SharedCRO';
...
export default function PdpStorytelling({ data, product, variant = 1 }: PDPProps) {
      ...
      <main className={layout.main}>
        ...
        {variant === 5 ? (
          <div className={layout.hero}>
            ...
            <div className="relative w-full aspect-square rounded-2xl shadow-lg mt-8 overflow-hidden">
          ...
          <div className={layout.hero}>
            <div className={`relative w-full ${variant === 6 ? 'aspect-square rounded-full' : variant === 8 ? 'aspect-[21/9]' : 'aspect-[4/3]'} overflow-hidden`}>
              <Image src={product.imageUrl} alt={product.title} fill className="object-cover" referrerPolicy="no-referrer" />
            </div>
            {variant === 1 && (
        ...
        <div className={`${layout.solution} ${isDark ? 'bg-zinc-950 border-zinc-800' : variant !== 5 && variant !== 9 && variant !== 3 && variant !== 7 ? 'bg-white border-zinc-100' : ''}`}>
          <div className={variant === 2 ? 'order-last' : ''}>
            <h3 className={`text-3xl font-bold ${isDark || variant === 5 || variant === 9 ? 'text-white' : 'text-zinc-900'} mb-6 flex items-center ${variant === 1 || variant === 5 || variant === 6 || variant === 10 ? 'justify-center' : ''} space-x-3`}>
              ...
              <span>Presentamos {product.title}</span>
            </h3>
        ...
        <div className="text-center mt-16 mb-16">
          ...
          >
            <span>Quiero mi {product.title} ahora</span>
