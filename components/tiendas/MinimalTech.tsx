    ...
    <div className="min-h-full bg-[#f5f5f7] font-sans text-[#1d1d1f] selection:bg-[#0071e3] selection:text-white pb-0 overflow-x-hidden">
        ...
        <div className="w-full max-w-[1024px] mx-auto px-4 h-[44px] flex items-center justify-between text-[12px] text-gray-800 font-medium">
          ...
          <div className="flex items-center space-x-4 lg:space-x-8">
            ...
            >
              ...
              {itemCount > 0 && (
                <span className="absolute -top-2 -right-2 bg-[#0071e3] text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{itemCount}</span>
              )}
            </div>
            <button 
      ...
      <main className="w-full">
        ...
        <section className="py-12 px-4 max-w-[1024px] mx-auto">
          <h3 className="text-[24px] md:text-[32px] font-semibold tracking-tight mb-2">New Arrivals</h3>
          <p className="text-[17px] text-gray-500 mb-8">The latest. Take a look at what's new, right now.</p>
          <div className="flex overflow-x-auto gap-5 pb-4 snap-x snap-mandatory hide-scrollbar -mx-4 px-4">
            {allProducts.slice(0, 6).map((product, idx) => (
              <div key={`mt-new-${idx}`} data-product-id={product.id} className="group cursor-pointer shrink-0 w-[260px] snap-start">
                <div className="relative aspect-square bg-white rounded-2xl overflow-hidden mb-3 shadow-sm group-hover:shadow-md transition-shadow">
                  <Image src={product.imageUrl} alt={product.title} fill className="object-contain p-6 group-hover:scale-105 transition-transform duration-300" />
                  {product.discount && product.discount > 0 && (
                    <span className="absolute top-3 left-3 bg-[#bf4800] text-white text-[11px] font-bold px-2 py-0.5 rounded-full">New</span>
                  )}
                </div>
                <h4 className="text-[14px] font-medium text-[#1d1d1f] truncate">{product.title}</h4>
                <p className="text-[14px] text-[#bf4800]">From ${product.price}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="py-10 bg-white">
          <div className="max-w-[1024px] mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              { icon: <CreditCard className="w-8 h-8 text-[#0071e3]" />, title: 'Pay Monthly', desc: 'Pay over time, interest-free.' },
              { icon: <Box className="w-8 h-8 text-[#0071e3]" />, title: 'Free Delivery', desc: 'Get free delivery on all orders.' },
              { icon: <HeadphonesIcon className="w-8 h-8 text-[#0071e3]" />, title: 'Expert Support', desc: 'Get help from specialists.' },
              { icon: <PlayCircle className="w-8 h-8 text-[#0071e3]" />, title: 'Trade In', desc: 'Upgrade and save with trade-in.' },
            ].map((svc, i) => (
              <div key={i} className="flex flex-col items-center text-center group cursor-pointer">
                <div className="mb-3 group-hover:scale-110 transition-transform">{svc.icon}</div>
                <h4 className="text-[14px] font-semibold text-[#1d1d1f] mb-1">{svc.title}</h4>
                <p className="text-[12px] text-gray-500">{svc.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="py-12 px-4 max-w-[1024px] mx-auto">
          <h3 className="text-[24px] md:text-[32px] font-semibold tracking-tight mb-2">Accessories</h3>
          <p className="text-[17px] text-gray-500 mb-8">Explore cases, covers, AirTags, and more.</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {accessories.map((product, idx) => (
              <div key={`mt-acc-${idx}`} data-product-id={product.id} className="group cursor-pointer bg-[#fbfbfd] rounded-2xl p-4 hover:shadow-md transition-shadow">
                <div className="relative aspect-square mb-3 overflow-hidden rounded-xl">
                  <Image src={product.imageUrl} alt={product.title} fill className="object-contain p-4 group-hover:scale-105 transition-transform duration-300" />
                </div>
                <h4 className="text-[14px] font-medium text-[#1d1d1f] truncate">{product.title}</h4>
                <p className="text-[14px] text-gray-500">${product.price}</p>
                <button onClick={(e) => handleAddToCart(product, e)} className="mt-2 text-[#0066cc] text-[13px] font-medium hover:underline">Add to Bag</button>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* ─── EXPERIENCE BANNER ─── */}
        <section className="relative w-full h-[400px] md:h-[500px] bg-black text-white flex flex-col items-center justify-center cursor-pointer overflow-hidden">
          <Image src="https://picsum.photos/1920/800?random=310" alt="Experience" fill className="object-cover opacity-50 -z-10" referrerPolicy="no-referrer" />
          <div className="text-center z-10 px-4">
            <p className="text-[14px] uppercase tracking-[0.2em] mb-3 text-gray-300">The MinimalTech Experience</p>
            <h2 className="text-[32px] md:text-[48px] font-semibold tracking-tight mb-4">Visit a Store</h2>
            <p className="text-[17px] text-gray-300 mb-6 max-w-md mx-auto">Shop one-on-one with a Specialist. Get help from a Genius. Enjoy a Today at MinimalTech session.</p>
            <a href="#" className="text-[#2997ff] hover:underline text-[17px] flex items-center justify-center">Find a store near you <ChevronRight className="w-4 h-4 ml-1" /></a>
          </div>
        </section>
