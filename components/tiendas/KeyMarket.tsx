  ...
  return (
    <div className="min-h-full bg-stone-50 font-sans text-stone-900 selection:bg-orange-500 selection:text-white antialiased">
      ...
      <main className="max-w-[1440px] mx-auto px-6 py-8">
        ...
        {/* ─── TOP SELLERS ─── */}
        <section className="mb-24">
          <h2 className="text-4xl font-black uppercase tracking-tighter mb-12 flex items-center gap-4"><Star className="w-10 h-10 text-orange-500 fill-orange-500" /> TOP RATED SELLERS</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {data.products.slice(0, 4).map((product, idx) => (
              <div key={`top-${product.id || idx}`} className="group cursor-pointer bg-white border border-stone-100 rounded-2xl p-6 hover:border-orange-500 hover:shadow-xl transition-all">
                <div className="relative aspect-video bg-black rounded-xl overflow-hidden mb-4">
                  <img src={product.imageUrl} alt={product.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  <button onClick={(e) => { e.stopPropagation(); toggleFavorite(product.id); }} className="absolute top-3 right-3 p-2 bg-black/60 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                    <Heart className={`w-4 h-4 ${favorites.includes(product.id) ? 'fill-orange-500 text-orange-500' : 'text-white'}`} />
                  </button>
                </div>
                <h3 className="font-black text-sm uppercase line-clamp-1 mb-2">{product.title}</h3>
                <div className="flex items-center justify-between">
                  <span className="text-xl font-black">${product.price}</span>
                  <button onClick={(e) => handleAddToCart(product, e)} className="bg-orange-500 text-white p-2 rounded-lg hover:bg-black transition-colors">
        ...
        {/* ─── PLATFORM STATS ─── */}
        <section className="mb-24 bg-white rounded-[3rem] border border-stone-100 p-12 shadow-xl">
          <h2 className="text-3xl font-black uppercase tracking-tighter text-center mb-12">Platform Statistics</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {[
              ...
              <div key={i} className="p-6">
                ...
                <div className="text-[11px] font-black uppercase tracking-widest text-stone-400">{stat.label}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* ─── HOW IT WORKS ─── */}
        <section className="mb-24">
          <h2 className="text-4xl font-black uppercase tracking-tighter text-center mb-16">How KeyMarket Works</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { step: '01', icon: Search, title: 'Find Your Key', desc: 'Browse thousands of verified digital products from trusted sellers.' },
              { step: '02', icon: Lock, title: 'Secure Purchase', desc: 'Pay safely with our escrow system. Funds held until key is verified.' },
              { step: '03', icon: Download, title: 'Instant Delivery', desc: 'Receive your digital key instantly via email. Activate and enjoy.' },
            ].map((item, i) => (
              <div key={i} className="bg-white border border-stone-100 rounded-[2rem] p-10 text-center hover:border-orange-500 hover:shadow-xl transition-all">
                <div className="text-5xl font-black text-orange-500/20 mb-4">{item.step}</div>
                <div className="w-16 h-16 bg-orange-50 rounded-2xl flex items-center justify-center mx-auto mb-6">
                  <item.icon className="w-8 h-8 text-orange-500" />
                </div>
                <h3 className="text-xl font-black uppercase tracking-tighter mb-3">{item.title}</h3>
                <p className="text-stone-400 font-bold text-[11px] uppercase tracking-widest">{item.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* ─── CUSTOMER REVIEWS ─── */}
        <section className="mb-24 bg-stone-50 rounded-[3rem] p-12 border border-stone-100">
          <h2 className="text-3xl font-black uppercase tracking-tighter text-center mb-12">What Gamers Say</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { name: 'Alex G.', text: 'Best prices for game keys. Instant delivery every time. Been using KeyMarket for 3 years now.' },
              { name: 'Maria S.', text: 'The escrow system gives me peace of mind. Never had an issue with any purchase.' },
              { name: 'Tom R.', text: 'KeyMarket Plus is worth every penny. The monthly free game alone pays for the subscription.' },
            ].map((review, i) => (
              <div key={i} className="bg-white p-8 rounded-2xl border border-stone-100">
                ...
                <p className="text-stone-600 text-sm leading-relaxed mb-6">&ldquo;{review.text}&rdquo;</p>
        ...
        {/* ─── NEWSLETTER ─── */}
        <section className="mb-24 bg-black rounded-[3rem] p-12 md:p-20 text-white text-center">
          <Zap className="w-10 h-10 mx-auto mb-6 text-orange-500 fill-orange-500" />
          <h2 className="text-3xl font-black uppercase tracking-tighter mb-4">Get Deal Alerts</h2>
          <p className="text-stone-400 font-bold text-sm mb-10 max-w-md mx-auto">Subscribe to get notified when your wishlist items go on sale.</p>
          <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <input type="email" placeholder="YOUR EMAIL" className="flex-1 bg-stone-900 border border-stone-800 text-white px-6 py-4 rounded-xl font-bold text-[12px] tracking-widest placeholder-stone-600 outline-none focus:border-orange-500 transition-all" />
            <button className="bg-orange-500 text-white px-8 py-4 rounded-xl font-black uppercase text-[12px] tracking-widest hover:bg-white hover:text-black transition-all">Subscribe</button>
          </div>
        </section>
      </main>
