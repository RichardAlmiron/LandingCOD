  ...
  return (
    <div className="min-h-full bg-white font-sans text-[#333]">
        ...
        <div className="w-full mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-6 text-[#1c4396]">
            ...
            >
              ...
              <span className="absolute top-0 right-0 bg-[#f8c134] text-[#1c4396] text-[10px] font-black px-1.5 rounded-full border border-white">{itemCount}</span>
      ...
      <main className="w-full mx-auto px-6 py-8">
        ...
        {/* Pet Care Tips */}
        <div className="mb-16">
          <h2 className="text-2xl font-black text-[#1c4396] mb-8 text-center">Pet Care Tips</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { title: "Best Foods for Senior Dogs", img: "https://picsum.photos/600/400?random=210" },
              { title: "How to Groom Your Cat at Home", img: "https://picsum.photos/600/400?random=211" },
              { title: "Setting Up Your First Aquarium", img: "https://picsum.photos/600/400?random=212" },
            ].map((tip, i) => (
              <div key={i} className="group cursor-pointer bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow">
                <div className="relative aspect-[3/2] overflow-hidden">
                  <img src={tip.img} alt={tip.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                </div>
                <div className="p-6">
                  <h3 className="font-black text-lg text-[#1c4396] mb-2 group-hover:underline">{tip.title}</h3>
                  <span className="text-sm font-bold text-[#1c4396] flex items-center">Read More <ChevronRight className="w-4 h-4 ml-1" /></span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Customer Reviews */}
        <div className="mb-16 bg-[#eef3fb] rounded-2xl p-8 md:p-12 border border-[#d2e0f5]">
          <h2 className="text-2xl font-black text-[#1c4396] mb-8 text-center">What Pet Parents Say</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { name: "Sarah M.", text: "PetFriend has the best selection and fastest delivery. My dog loves the treats!", rating: 5 },
              { name: "Mike R.", text: "Autoship saves me so much time. Never running out of cat food again.", rating: 5 },
              { name: "Lisa K.", text: "The pharmacy service is incredible. Easy to get prescriptions filled online.", rating: 5 },
            ].map((review, i) => (
              <div key={i} className="bg-white rounded-xl p-6 shadow-sm">
                <div className="flex mb-3">
                  {Array.from({ length: review.rating }).map((_, s) => (
                    <Star key={s} className="w-4 h-4 fill-[#f8c134] text-[#f8c134]" />
                  ))}
                </div>
                <p className="text-sm text-gray-700 leading-relaxed mb-3">&ldquo;{review.text}&rdquo;</p>
                <span className="text-sm font-black text-[#1c4396]">{review.name}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Give Back */}
        <div className="mb-16 bg-[#1c4396] text-white rounded-2xl p-8 md:p-12 text-center">
          <h2 className="text-3xl font-black mb-4">PetFriend Gives Back</h2>
          <p className="text-lg font-medium text-blue-200 mb-6 max-w-2xl mx-auto">
            We've donated over $300 million to animal welfare organizations. Every purchase helps pets in need find loving homes.
          </p>
          <p className="text-5xl font-black text-[#f8c134] mb-6">$300M+</p>
          <button className="bg-[#f8c134] text-[#1c4396] px-8 py-3 rounded-full font-black hover:bg-white transition-colors">
            Learn More
          </button>
        </div>
        ...
        <div className="mb-16 text-center">
          <h2 className="text-2xl font-black text-[#1c4396] mb-4">Stay in the Loop</h2>
          <p className="text-sm text-gray-500 mb-6">Get exclusive deals, pet care tips, and new product alerts.</p>
          <div className="flex flex-col sm:flex-row gap-3 max-w-[500px] mx-auto">
            <input type="email" placeholder="Email Address" className="flex-1 border-2 border-[#1c4396] px-6 py-3 rounded-full outline-none focus:border-[#f8c134] transition-colors" />
            <button className="bg-[#f8c134] text-[#1c4396] px-8 py-3 rounded-full font-black hover:bg-[#1c4396] hover:text-white transition-colors">Subscribe</button>
          </div>
        </div>
