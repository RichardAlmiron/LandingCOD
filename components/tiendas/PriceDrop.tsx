...
export default function PriceDropTemplate({ data }: { data: StoreData }) {
  ...
  return (
    <div className="min-h-full bg-[#f4f4f4] font-sans text-[#333] selection:bg-[#ff6000] selection:text-white pb-10 overflow-x-hidden">
          ...
          <div className="flex items-center h-full space-x-4 ml-4 md:ml-8">
            ...
            >
              <div className="relative">
                ...
                {itemCount > 0 && (
                  <span className="absolute -top-1.5 -right-2 bg-[#ff6000] text-white text-[11px] font-bold px-1.5 rounded-full min-w-[20px] text-center border-2 border-white shadow-sm">{itemCount}</span>
                )}
              </div>
        ...
        <div className="mb-10 lg:mb-14">
          ...
          <div className="flex overflow-x-auto hide-scrollbar gap-2 sm:gap-4 pb-4 px-1" style={{ scrollSnapType: 'x mandatory' }}>
            {data.products.slice(0, 10).map((product, idx) => (
              <div key={idx} data-product-id={product.id} data-discount={product.discount || 0} className="shrink-0 w-[140px] md:w-[180px] lg:w-[220px] bg-white rounded-[8px] p-2 md:p-3 hover:shadow-[0_4px_12px_rgba(0,0,0,0.08)] transition-all cursor-pointer border border-gray-100 flex flex-col hover:border-[#ff6000]" style={{ scrollSnapAlign: 'start' }}>
                ...
                <div className="flex flex-col mt-auto pb-1">
                  ...
                  <div className="flex items-baseline gap-1.5 mt-1">
                    ...
                    {product.originalPrice && (
                      <span className="text-[11px] md:text-[12px] text-gray-400 line-through font-medium leading-none">${product.originalPrice}</span>
                    )}
                  </div>
                </div>
              </div>
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-2 sm:gap-3 md:gap-4">
            {paginatedItems.map((product: any, idx: number) => (
              <div key={product.id || idx} data-product-id={product.id} data-discount={product.discount || 0} className="group cursor-pointer flex flex-col bg-white rounded-[8px] overflow-hidden hover:shadow-[0_8px_16px_rgba(0,0,0,0.1)] transition-all border border-transparent hover:border-gray-200">
                <div className="relative aspect-square overflow-hidden bg-[#f7f7f7]">
                  ...
                  />
                  {product.discount && product.discount > 0 && (
                    <div className="absolute top-2 left-2 bg-[#ff6000] text-white text-[10px] font-bold px-2 py-1 rounded-[4px] shadow">
                      -{product.discount}%
                    </div>
                  )}
                  {/* Cart Overlay Button */}
                ...
                <div className="p-2 sm:p-3 flex flex-col flex-1 h-[150px]">
                  ...
                  <div className="mt-auto flex flex-col">
                    <div className="flex items-baseline space-x-1.5 overflow-hidden whitespace-nowrap">
                      ...
                      {product.originalPrice && (
                        <span className="text-[11px] sm:text-[13px] text-gray-400 line-through font-medium leading-none">${product.originalPrice}</span>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {/* ─── SHOP BY CATEGORY ─── */}
      <div className="w-full max-w-[1500px] mx-auto px-4 mb-16">
        <h2 className="text-[22px] md:text-[28px] font-black tracking-tight text-gray-900 mb-6 px-1">Shop by Category</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {[
            { name: 'Women', img: 'https://picsum.photos/300/300?random=192' },
            { name: 'Men', img: 'https://picsum.photos/300/300?random=193' },
            { name: 'Home', img: 'https://picsum.photos/300/300?random=194' },
            { name: 'Electronics', img: 'https://picsum.photos/300/300?random=195' },
            { name: 'Beauty', img: 'https://picsum.photos/300/300?random=196' },
            { name: 'Kids', img: 'https://picsum.photos/300/300?random=197' },
          ].map((cat, i) => (
            <div key={i} className="bg-white rounded-[12px] p-4 flex flex-col items-center cursor-pointer hover:shadow-lg transition-all border border-gray-100 group">
              <div className="w-20 h-20 rounded-full overflow-hidden mb-3 bg-gray-50 group-hover:scale-110 transition-transform">
                <Image src={cat.img} alt={cat.name} width={80} height={80} className="w-full h-full object-cover" />
              </div>
              <span className="font-bold text-sm text-gray-700 group-hover:text-[#ff6000]">{cat.name}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* ─── TRENDING NOW ─── */}
      <div className="w-full max-w-[1500px] mx-auto px-4 mb-16">
        <h2 className="text-[22px] md:text-[28px] font-black tracking-tight text-gray-900 mb-6 px-1 flex items-center">
          <Flame className="w-6 h-6 mr-2 fill-[#ff6000] text-[#ff6000]" /> Trending Now
        </h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { name: "Summer Dresses", img: "https://picsum.photos/400/500?random=198" },
            { name: "Smart Watches", img: "https://picsum.photos/400/500?random=199" },
            { name: "Home Decor", img: "https://picsum.photos/400/500?random=200" },
            { name: "Sneakers", img: "https://picsum.photos/400/500?random=201" },
          ].map((item, i) => (
            <div key={i} className="relative aspect-[4/5] rounded-[12px] overflow-hidden group cursor-pointer">
      ...
      <div className="w-full max-w-[1500px] mx-auto px-4 mb-16">
        <h2 className="text-[22px] md:text-[28px] font-black tracking-tight text-gray-900 mb-6 px-1 flex items-center">
          <Star className="w-6 h-6 mr-2 fill-yellow-400 text-yellow-400" /> Customer Reviews
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { name: "Maria G.", text: "Incredible prices! I ordered 10 items and everything arrived in great condition.", rating: 5 },
            { name: "James W.", text: "The quality is surprisingly good for the price. Free shipping is a huge plus.", rating: 5 },
            { name: "Ashley P.", text: "My new favorite shopping app. The deals are unbeatable and returns are easy.", rating: 5 },
          ].map((review, i) => (
            <div key={i} className="bg-white rounded-[12px] p-6 border border-gray-100 shadow-sm">
              <div className="flex mb-3">
                {Array.from({ length: review.rating }).map((_, s) => (
                  <Star key={s} className="w-4 h-4 fill-yellow-400 text-yellow-400" />
                ))}
              </div>
              <p className="text-[14px] text-gray-700 leading-relaxed mb-3">&ldquo;{review.text}&rdquo;</p>
              <span className="text-[13px] font-black text-gray-900">{review.name}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* ─── NEW USER BENEFITS ─── */}
      <div className="w-full max-w-[1500px] mx-auto px-4 mb-16">
        <div className="bg-gradient-to-r from-[#ff6000] to-[#ff8533] rounded-[16px] p-8 md:p-12 text-white text-center">
          <h2 className="text-[28px] md:text-[36px] font-black mb-4">New User? Welcome Gift!</h2>
          <p className="text-[16px] md:text-[18px] font-medium mb-8 opacity-90 max-w-xl mx-auto">
            Sign up today and get a $100 coupon bundle plus free shipping on your first 3 orders.
          </p>
          <button className="bg-white text-[#ff6000] px-10 py-4 rounded-[40px] font-black text-[16px] hover:scale-105 transition-transform shadow-xl">
            Claim Your Gift
          </button>
        </div>
      </div>
      ...
      {/* ─── BRAND PARTNERS ─── */}
      <div className="w-full max-w-[1500px] mx-auto px-4 mb-16">
        <h2 className="text-[22px] md:text-[28px] font-black tracking-tight text-gray-900 mb-6 px-1">Featured Brands</h2>
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
          {['Brand A', 'Brand B', 'Brand C', 'Brand D', 'Brand E', 'Brand F'].map((brand, i) => (
            <div key={i} className="bg-white rounded-[12px] p-6 flex items-center justify-center border border-gray-100 hover:shadow-md transition-shadow cursor-pointer h-20">
              <span className="font-black text-lg text-gray-300 hover:text-[#ff6000] transition-colors">{brand}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* ─── NEWSLETTER ─── */}
      <div className="w-full max-w-[1500px] mx-auto px-4 mb-16 text-center">
        <h2 className="text-[22px] md:text-[28px] font-black tracking-tight text-gray-900 mb-4">Stay Updated</h2>
        <p className="text-[14px] text-gray-500 mb-6">Get exclusive deals and new arrivals straight to your inbox.</p>
        <div className="flex flex-col sm:flex-row gap-3 max-w-[500px] mx-auto">
          <input type="email" placeholder="Email Address" className="flex-1 bg-white border-2 border-[#ff6000] px-6 py-3.5 rounded-[40px] outline-none font-bold" />
          <button className="bg-[#ff6000] text-white px-8 py-3.5 rounded-[40px] font-black hover:bg-[#e55600] transition-colors">Subscribe</button>
        </div>
      </div>
