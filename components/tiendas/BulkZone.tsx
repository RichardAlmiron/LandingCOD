  ...
  return (
    <div className="min-h-full bg-[#f2f2f2] font-sans text-stone-800">
        ...
        <div className="max-w-[1440px] mx-auto px-6 py-5 flex items-center gap-10">
          ...
          <div className="flex items-center gap-6 lg:gap-8 font-extrabold text-[#005daa]">
            ...
            >
              ...
              {itemCount > 0 && (
                <span className="absolute -top-1 -right-2 bg-[#e31837] text-white text-[10px] font-black px-1.5 py-0.5 rounded-full ring-2 ring-white">{itemCount}</span>
              )}
      ...
      <main className="max-w-[1440px] mx-auto px-6 py-8">
        ...
        <div className="mb-12 bg-white border border-stone-200 rounded-lg p-8 md:p-10 flex flex-col md:flex-row items-center gap-8">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              <div className="bg-[#005daa]/10 p-3 rounded-full">
                <Truck className="w-8 h-8 text-[#005daa]" />
              </div>
              <h2 className="text-2xl font-black uppercase tracking-tight text-stone-900">Same-Day Delivery</h2>
            </div>
            <p className="text-stone-600 text-sm font-medium mb-6 max-w-lg leading-relaxed">
              Order by 3pm and get your essentials delivered to your door the same day. Available on thousands of items for Gold Star and Executive members.
            </p>
            <div className="flex gap-4">
              <button className="bg-[#005daa] text-white px-8 py-3 font-black text-xs uppercase tracking-widest hover:bg-[#004d8a] transition-all rounded shadow-sm">
                Shop Eligible Items
              </button>
              <button className="border-2 border-stone-300 text-stone-700 px-8 py-3 font-black text-xs uppercase tracking-widest hover:border-stone-900 transition-all rounded">
                Learn More
              </button>
            </div>
          </div>
          <div className="hidden md:grid grid-cols-3 gap-4 text-center">
            {[
              { label: 'Order by', value: '3 PM' },
              { label: 'Delivered by', value: '8 PM' },
              { label: 'Min. order', value: '$35' },
            ].map((stat, i) => (
              <div key={i} className="bg-stone-50 p-4 rounded-lg border border-stone-200">
                <div className="text-2xl font-black text-[#005daa]">{stat.value}</div>
                <div className="text-[10px] font-bold text-stone-500 uppercase tracking-widest mt-1">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mb-12">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-black uppercase tracking-tight text-stone-900">Trending This Week</h2>
            <a href="#" className="text-sm font-black text-[#005daa] hover:underline uppercase tracking-widest flex items-center">
              See All <ArrowRight className="w-4 h-4 ml-1" />
            </a>
          </div>
          <div className="flex overflow-x-auto gap-4 pb-4 snap-x hide-scrollbar">
            {data.products.slice(0, 6).map((product, idx) => (
              <div key={`trend-${idx}`} className="shrink-0 w-[220px] snap-start bg-white border border-stone-200 rounded-lg p-4 group cursor-pointer hover:shadow-lg transition-shadow">
                <div className="relative aspect-square mb-4 flex items-center justify-center">
                  <img src={product.imageUrl} alt={product.title} className="max-w-full max-h-full object-contain mix-blend-multiply group-hover:scale-105 transition-transform" />
                  <div className="absolute top-0 left-0 bg-[#e31837] text-white text-[9px] font-black px-2 py-0.5 rounded uppercase">Hot</div>
                </div>
                <h3 className="text-xs font-bold text-[#005daa] line-clamp-2 mb-2 group-hover:underline">{product.title}</h3>
                <div className="flex items-end justify-between">
                  <span className="text-lg font-black text-stone-900">${product.price}</span>
                  <div className="flex text-amber-500">
                    {[1,2,3,4,5].map(v => <Star key={v} className="w-2.5 h-2.5 fill-current" />)}
                  </div>
                </div>
              </div>
            ))}
        ...
        {/* ─── RECENTLY VIEWED ─── */}
        <div className="mb-12">
          <h2 className="text-2xl font-black uppercase tracking-tight text-stone-900 mb-6">Recently Viewed</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {data.products.slice(0, 4).map((product, idx) => (
              <div key={`rv-${idx}`} className="bg-white border border-stone-200 rounded-lg p-4 group cursor-pointer hover:shadow-md transition-shadow">
                <div className="relative aspect-[4/3] mb-3 flex items-center justify-center">
                  <img src={product.imageUrl} alt={product.title} className="max-w-full max-h-full object-contain mix-blend-multiply" />
                </div>
                <h3 className="text-xs font-bold text-stone-700 line-clamp-1 mb-1 group-hover:underline">{product.title}</h3>
