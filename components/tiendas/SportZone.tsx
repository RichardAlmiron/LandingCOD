  ...
  return (
    <div className="min-h-full bg-[#f4f5f7] font-sans text-[#333333] selection:bg-[#0082c3] selection:text-white pb-10 overflow-x-hidden">
          ...
          <div className="flex items-center space-x-2 md:space-x-5 text-[#333]">
            ...
            >
              <div className="relative">
                ...
                {itemCount > 0 && (
                  <span className="absolute -top-1.5 -right-2 bg-[#ffd100] text-black text-[11px] font-black w-[20px] h-[20px] flex items-center justify-center rounded-full leading-none border-2 border-white shadow-sm">{itemCount}</span>
                )}
              </div>
      ...
      <main className="w-full max-w-[1400px] mx-auto px-4 lg:px-6 py-6 md:py-8">
          ...
          <div className="mb-16 md:mb-20">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-5">
              {paginatedItems.map((product: any, idx: number) => (
                <div key={product.id || idx} data-product-id={product.id} data-discount={product.discount || 0} className="group cursor-pointer flex flex-col bg-white border border-gray-200 rounded-[4px] hover:border-[#0082c3] hover:shadow-[0_8px_20px_rgba(0,130,195,0.15)] transition-all h-full relative">
                  {product.discount && product.discount > 0 && <div className="absolute top-0 right-0 z-10 bg-[#cc163f] text-white text-[10px] md:text-[12px] font-black px-2 py-1 uppercase italic shadow-sm">-{product.discount}% OFF</div>}
                  {product.discount && product.discount > 15 && <div className="absolute top-0 left-0 z-10 bg-[#ffd100] text-black text-[10px] md:text-[12px] font-black px-2 py-1 uppercase italic shadow-sm">BEST SELLER</div>}
                  ...
                  <div className="p-4 flex flex-col flex-1 border-t border-gray-100">
                    ...
                    <div className="mt-auto flex flex-col pt-2">
                      <div className="flex text-[#ffd100] mb-2">
                        ...
                        <span className="text-[11px] text-gray-500 font-bold ml-1">({product.reviews || 0})</span>
                      </div>
                      <div className="flex items-end flex-wrap gap-2">
                        <span className="font-black text-[22px] md:text-[24px] text-[#333] leading-none">${product.price}</span>
                        {product.originalPrice && <span className="text-[13px] text-gray-400 line-through font-bold mb-0.5">${product.originalPrice}</span>}
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div className="mb-20">
          <h2 className="text-[24px] md:text-[28px] font-black text-[#333] uppercase italic mb-8 flex items-center border-b-2 border-[#f4f5f7] pb-2">
            Seasonal Sports Spotlight
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { title: 'Trail Running', subtitle: 'Conquer every terrain', img: 'https://picsum.photos/800/600?random=220' },
              { title: 'Road Cycling', subtitle: 'Speed meets endurance', img: 'https://picsum.photos/800/600?random=221' },
              { title: 'Open Water Swimming', subtitle: 'Dive into adventure', img: 'https://picsum.photos/800/600?random=222' },
            ].map((item, i) => (
              <div key={i} className="relative h-[320px] rounded-[4px] overflow-hidden group cursor-pointer shadow-md">
                ...
                <div className="absolute bottom-6 left-6 text-white">
                  <h3 className="font-black text-[22px] uppercase italic mb-1">{item.title}</h3>
                  <p className="text-[14px] font-medium opacity-80">{item.subtitle}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* ─── BRAND PARTNERS ─── */}
        <div className="mb-20 bg-white border border-gray-200 rounded-[4px] p-8 md:p-12 shadow-sm">
          <h2 className="text-[24px] md:text-[28px] font-black text-[#333] uppercase italic mb-8 text-center">Our Brand Partners</h2>
          <div className="grid grid-cols-3 md:grid-cols-6 gap-8 items-center justify-items-center opacity-60">
            {['Adidas', 'Nike', 'Puma', 'Reebok', 'Asics', 'New Balance'].map((brand, i) => (
              <div key={i} className="flex flex-col items-center gap-2 hover:opacity-100 transition-opacity cursor-pointer">
                <div className="w-[80px] h-[80px] bg-[#f4f5f7] rounded-full flex items-center justify-center">
                  <Image src={`https://picsum.photos/80/80?random=${230 + i}`} alt={brand} width={50} height={50} className="object-contain rounded-full" />
                </div>
                <span className="text-[12px] font-bold text-[#333] uppercase tracking-wide">{brand}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* ─── MEMBER BENEFITS ─── */}
        <div className="mb-20 bg-[#0082c3] rounded-[4px] p-8 md:p-14 text-white text-center shadow-lg">
          <h2 className="text-[32px] md:text-[40px] font-black uppercase italic mb-4">Join SportZone Club</h2>
          <p className="text-[16px] md:text-[18px] font-medium opacity-90 max-w-2xl mx-auto mb-10">
            Become a member for free and unlock exclusive benefits: unlimited returns, early access to sales, personalized offers, and more.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto mb-10">
            <div className="bg-white/10 backdrop-blur-sm p-6 rounded-[4px]">
              <h3 className="font-black text-[36px] italic leading-none mb-2">365</h3>
              <p className="text-[14px] font-bold uppercase tracking-wide opacity-80">Days to Return</p>
            </div>
            <div className="bg-white/10 backdrop-blur-sm p-6 rounded-[4px]">
              <h3 className="font-black text-[36px] italic leading-none mb-2">10%</h3>
              <p className="text-[14px] font-bold uppercase tracking-wide opacity-80">Birthday Discount</p>
            </div>
            <div className="bg-white/10 backdrop-blur-sm p-6 rounded-[4px]">
              <h3 className="font-black text-[36px] italic leading-none mb-2">Free</h3>
              <p className="text-[14px] font-bold uppercase tracking-wide opacity-80">Workshop Access</p>
            </div>
          </div>
          <button className="bg-white text-[#0082c3] px-10 py-4 rounded-[4px] font-black text-[16px] hover:bg-[#ffd100] hover:text-black transition-colors uppercase italic shadow-md">
            Sign Up Free
          </button>
        </div>

        {/* ─── STORE LOCATOR BANNER ─── */}
        <div className="mb-20 flex flex-col lg:flex-row bg-white border border-gray-200 rounded-[4px] overflow-hidden shadow-sm">
          <div className="w-full lg:w-1/2 relative h-[300px] lg:h-auto">
            <Image src="https://picsum.photos/1000/600?random=225" alt="Store" fill className="object-cover" />
          </div>
          <div className="w-full lg:w-1/2 p-8 md:p-12 flex flex-col justify-center">
            <div className="flex items-center text-[#0082c3] font-black text-[13px] uppercase tracking-widest mb-4">
              <MapPin className="w-5 h-5 mr-2" /> Find Your Store
            </div>
            <h2 className="text-[28px] md:text-[36px] font-black text-[#333] uppercase italic mb-4 leading-tight">1,700+ Stores Worldwide</h2>
            <p className="text-[16px] text-gray-600 font-medium mb-8 leading-relaxed">
              Visit your nearest SportZone store for expert advice, product testing, and exclusive in-store services like bike fitting and racket stringing.
            </p>
            <button className="bg-[#0082c3] text-white px-8 py-3.5 rounded-[4px] font-black text-[15px] hover:bg-[#0070a8] transition-colors uppercase italic self-start flex items-center">
              <MapPin className="w-4 h-4 mr-2" /> Find a Store Near You
            </button>
          </div>
        </div>
