  ...
  return (
    <div className="min-h-full bg-white font-sans text-[#333333]">
        ...
        <div className="w-full mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-6 text-[#005a9c]">
            ...
            >
              ...
              {itemCount > 0 && (
                <span className="absolute -top-2 -right-2 bg-[#005a9c] text-white text-[9px] font-bold px-1.5 rounded-full">{itemCount}</span>
              )}
            </div>
          </div>
        </div>
      ...
      <main className="w-full mx-auto px-6 py-12">
        ...
        <div className="mb-24">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-black text-[#333333] mb-2">Trending Frames</h2>
            <p className="text-sm font-sans text-gray-500">The styles everyone is wearing right now.</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              { name: "Aviator", img: "https://picsum.photos/500/400?random=398" },
              { name: "Cat Eye", img: "https://picsum.photos/500/400?random=399" },
              { name: "Round", img: "https://picsum.photos/500/400?random=400" },
              { name: "Rectangle", img: "https://picsum.photos/500/400?random=401" },
            ].map((frame, i) => (
              <div key={i} className="group cursor-pointer text-center">
                <div className="relative aspect-square mb-4 bg-[#f4f4f4] rounded-2xl overflow-hidden">
                  <Image src={frame.img} alt={frame.name} fill className="object-contain p-6 group-hover:scale-110 transition-transform duration-700" />
                </div>
                <h3 className="text-lg font-bold text-[#333333] group-hover:text-[#005a9c] transition-colors">{frame.name}</h3>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Customer Reviews */}
        <div className="mb-24 bg-[#f0f5fa] rounded-2xl p-12">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-black text-[#333333] mb-2">Customer Reviews</h2>
            <p className="text-sm font-sans text-gray-500">See what our customers have to say.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { name: "Robert P.", text: "Best optical experience I've ever had. The staff was incredibly helpful and knowledgeable.", rating: 5 },
              { name: "Linda S.", text: "Found the perfect frames in minutes. The virtual try-on feature is a game changer.", rating: 5 },
              { name: "Michael T.", text: "Great selection of designer brands at reasonable prices. Will definitely be back.", rating: 5 },
            ].map((review, i) => (
              <div key={i} className="bg-white rounded-xl p-8 shadow-sm">
                <div className="flex mb-3">
                  {Array.from({ length: review.rating }).map((_, s) => (
                    <span key={s} className="text-yellow-400 text-[16px]">★</span>
                  ))}
                </div>
                <p className="text-[14px] text-gray-700 leading-relaxed mb-4">&ldquo;{review.text}&rdquo;</p>
                <span className="text-[13px] font-bold text-[#333333]">{review.name}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Store Locator */}
        <div className="mb-24 relative rounded-2xl overflow-hidden h-[400px] group cursor-pointer">
          <Image src="https://picsum.photos/1400/500?random=402" alt="Store" fill className="object-cover group-hover:scale-105 transition-transform duration-[2000ms]" />
          <div className="absolute inset-0 bg-[#005a9c]/60" />
          <div className="absolute inset-0 flex flex-col items-center justify-center text-white text-center p-6">
            <MapPin className="w-12 h-12 mb-6" strokeWidth={1.5} />
            <h2 className="text-4xl font-black mb-4">Find a Store Near You</h2>
            <p className="text-lg font-medium mb-8 max-w-lg">Visit one of our 1,000+ locations for a personalized experience.</p>
            <button className="bg-white text-[#005a9c] px-10 py-4 font-bold text-[14px] hover:bg-gray-100 transition-all rounded-full">
              Store Locator
            </button>
          </div>
        </div>
        ...
        <div className="mb-24 text-center">
          <h2 className="text-3xl font-black text-[#333333] mb-4">Stay Connected</h2>
          <p className="text-sm text-gray-500 mb-8 max-w-md mx-auto">Sign up for exclusive offers, new arrivals, and vision care tips.</p>
          <div className="flex flex-col sm:flex-row gap-3 max-w-[500px] mx-auto">
            <input type="email" placeholder="Email Address" className="flex-1 bg-white border border-gray-300 px-6 py-3.5 rounded-full outline-none focus:border-[#005a9c] transition-colors" />
            <button className="bg-[#005a9c] text-white px-8 py-3.5 rounded-full font-bold hover:bg-[#003d6b] transition-colors">
              Subscribe
            </button>
          </div>
        </div>
