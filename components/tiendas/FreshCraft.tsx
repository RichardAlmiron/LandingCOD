    ...
    <div className="min-h-full bg-white font-sans text-black selection:bg-black selection:text-white overflow-x-hidden">
        ...
        <div className="w-full max-w-[1440px] mx-auto px-4 md:px-8 h-[72px] lg:h-[88px] flex items-center justify-between">
          ...
          <div className="flex items-center space-x-6 text-black h-full">
            ...
            >
              ...
              {itemCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-black text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{itemCount}</span>
              )}
            </div>
          </div>
        </div>
      ...
      <main className="w-full">
        ...
        <section className="max-w-[1440px] mx-auto px-6 py-16 md:py-20">
          <h2 className="text-[40px] md:text-[56px] font-black tracking-tighter text-black uppercase leading-[0.9] mb-10 text-center">Trending Right Now</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              { title: 'Sleepy Bubble Bar', tag: 'Best Seller', img: 'https://picsum.photos/500/600?random=1201' },
              { title: 'Honey I Washed The Kids', tag: 'Fan Favorite', img: 'https://picsum.photos/500/600?random=1202' },
              { title: 'Karma Perfume', tag: 'Iconic', img: 'https://picsum.photos/500/600?random=1203' },
              { title: 'Charity Pot Lotion', tag: 'Giving Back', img: 'https://picsum.photos/500/600?random=1204' },
            ].map((item, i) => (
              <div key={i} className="group cursor-pointer border-4 border-black rounded-xl overflow-hidden shadow-[4px_4px_0_0_#000] hover:-translate-y-1 transition-transform">
                <div className="relative aspect-square">
                  ...
                  <div className="absolute top-3 left-3 bg-[#e91e63] border-2 border-black text-white text-[10px] font-black px-2 py-0.5 uppercase tracking-widest shadow-[2px_2px_0_0_#000]">{item.tag}</div>
                </div>
                <div className="p-4 bg-white">
                  <h3 className="text-[18px] font-black uppercase tracking-tighter text-black">{item.title}</h3>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* ─── CUSTOMER REVIEWS ─── */}
        <section className="w-full bg-[#fff3e0] border-y-4 border-black py-16 md:py-20">
          <div className="max-w-[1440px] mx-auto px-6">
            <h2 className="text-[40px] md:text-[56px] font-black tracking-tighter text-black uppercase leading-[0.9] mb-10 text-center">Fresh Reviews</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { name: 'Luna M.', text: 'The bath bombs are incredible. My bathroom smells like a garden for hours after.', rating: 5 },
                { name: 'Oliver K.', text: 'Switched to FreshCraft shampoo bars and my hair has never been healthier. Plus zero plastic waste.', rating: 5 },
                { name: 'Mia R.', text: 'Charity Pot is my holy grail moisturizer. Love that proceeds go to grassroots charities.', rating: 4 },
              ].map((review, i) => (
                <div key={i} className="bg-white p-8 border-4 border-black shadow-[4px_4px_0_0_#000]">
                  <div className="flex mb-4">
                    {[1,2,3,4,5].map(s => (
                      <span key={s} className={`text-[20px] ${s <= review.rating ? 'text-[#e91e63]' : 'text-gray-300'}`}>★</span>
                    ))}
                  </div>
                  ...
                  <div className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">Verified Buyer</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ─── SUSTAINABILITY ─── */}
        <section className="max-w-[1440px] mx-auto px-6 py-16 md:py-20">
          <div className="flex flex-col md:flex-row bg-[#4caf50] border-4 border-black shadow-[8px_8px_0_0_#000] overflow-hidden">
            <div className="w-full md:w-1/2 p-10 md:p-16 flex flex-col justify-center text-white">
              <Leaf className="w-12 h-12 mb-6" strokeWidth={3} />
              <h2 className="text-[36px] md:text-[48px] font-black uppercase tracking-tighter leading-tight mb-6">Fighting Animal Testing</h2>
              <p className="text-[18px] font-bold text-white/90 mb-8 max-w-md leading-relaxed">
                We have been fighting against animal testing since before we opened our first shop. We test on willing human volunteers, never on animals.
              </p>
              <button className="bg-white text-[#4caf50] border-4 border-black px-8 py-4 font-black uppercase tracking-widest text-[13px] hover:bg-black hover:text-white transition-all w-fit shadow-[4px_4px_0_0_#000]">
                Our Ethics
              </button>
            </div>
            <div className="w-full md:w-1/2 relative h-[300px] md:h-auto">
              <Image src="https://picsum.photos/800/600?random=1210" alt="Sustainability" fill className="object-cover" />
            </div>
          </div>
        </section>
        ...
        {/* ─── APP DOWNLOAD ─── */}
        <section className="max-w-[1440px] mx-auto px-6 py-16 md:py-20">
          <div className="flex flex-col md:flex-row items-center bg-[#e1f5fe] border-4 border-black p-10 md:p-16 gap-10 shadow-[8px_8px_0_0_#000]">
            <div className="w-full md:w-1/2">
              <Sparkles className="w-12 h-12 mb-6 text-[#e91e63]" strokeWidth={3} />
              <h2 className="text-[36px] md:text-[48px] font-black uppercase tracking-tighter leading-tight mb-6">Get The FreshCraft App</h2>
              <p className="text-[18px] font-bold text-gray-600 mb-8 max-w-md leading-relaxed">
                Exclusive app-only products, virtual try-on for bath bombs, and early access to seasonal collections.
              </p>
              <div className="flex gap-4">
                <button className="bg-black text-white border-4 border-black px-8 py-4 font-black uppercase tracking-widest text-[12px] hover:bg-[#e91e63] transition-all shadow-[4px_4px_0_0_#000]">App Store</button>
                <button className="bg-black text-white border-4 border-black px-8 py-4 font-black uppercase tracking-widest text-[12px] hover:bg-[#e91e63] transition-all shadow-[4px_4px_0_0_#000]">Google Play</button>
              </div>
            </div>
            <div className="w-full md:w-1/2 flex justify-center">
              <div className="relative w-[220px] h-[440px] bg-black rounded-[36px] border-4 border-black overflow-hidden shadow-[8px_8px_0_0_#000]">
                <Image src="https://picsum.photos/400/800?random=1211" alt="App" fill className="object-cover" />
              </div>
            </div>
          </div>
        </section>
        ...
        {/* ─── SERVICES ─── */}
        <section className="w-full bg-black text-white border-y-4 border-black py-16 md:py-20">
          <div className="max-w-[1440px] mx-auto px-6">
            <h2 className="text-[40px] md:text-[56px] font-black tracking-tighter uppercase leading-[0.9] mb-12 text-center">The FreshCraft Promise</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
              {[
                { icon: '🌿', title: '100% Vegetarian', desc: 'Always and forever' },
                { icon: '🐰', title: 'Cruelty Free', desc: 'Never tested on animals' },
                { icon: '♻️', title: 'Naked Packaging', desc: 'Less waste, more product' },
                { icon: '🚚', title: 'Free Shipping', desc: 'On orders over $45' },
              ].map((s, i) => (
                <div key={i} className="flex flex-col items-center">
                  <div className="text-[40px] mb-4">{s.icon}</div>
                  <h3 className="text-[16px] font-black uppercase tracking-widest mb-2">{s.title}</h3>
                  <p className="text-[13px] font-bold text-gray-400">{s.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ─── NEWSLETTER ─── */}
        <section className="max-w-[1440px] mx-auto px-6 py-16 md:py-20 text-center">
          <h2 className="text-[40px] md:text-[56px] font-black tracking-tighter text-black uppercase leading-[0.9] mb-4">Join The Fresh Life</h2>
          <p className="text-[18px] font-bold text-gray-500 mb-10 max-w-lg mx-auto">Get 15% off your first order plus exclusive access to new products and events.</p>
          <div className="flex max-w-md mx-auto border-4 border-black shadow-[4px_4px_0_0_#000] overflow-hidden">
            <input type="email" placeholder="YOUR EMAIL" className="bg-white px-4 py-4 outline-none flex-1 font-black text-[13px] uppercase tracking-widest placeholder-gray-400" />
            <button className="bg-black text-white px-8 py-4 font-black uppercase tracking-widest text-[13px] hover:bg-[#e91e63] transition-colors shrink-0">
              Subscribe
            </button>
          </div>
        </section>
