  ...
  return (
    <div className="min-h-full bg-white font-sans text-[#000] selection:bg-[#f4a2b6] selection:text-white pb-0 overflow-x-hidden">
        ...
        <div className="w-full mx-auto px-4 md:px-8 h-full flex items-center justify-between">
          ...
          <div className="flex items-center space-x-5 lg:space-x-6 text-black">
            ...
            >
              ...
              {itemCount > 0 && (
                <span className="absolute -top-1.5 -right-2 bg-black group-hover:bg-[#f4a2b6] transition-colors text-white text-[11px] font-black px-1.5 rounded-full min-w-[20px] text-center shadow-sm">{itemCount}</span>
              )}
            </div>
          </div>
        </div>
      ...
      <main className="w-full">
        ...
        {/* ─── CUSTOMER REVIEWS ─── */}
        <section className="px-4 md:px-8 max-w-[1600px] mx-auto mb-20 md:mb-24">
          <h2 className="text-[36px] md:text-[48px] font-black uppercase tracking-tighter text-black leading-none mb-8 text-center">
            <Star className="w-8 h-8 inline fill-[#f4a2b6] text-[#f4a2b6] mr-2" /> What Babes Say
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { name: "Chloe T.", text: "Obsessed with the quality! Everything fits perfectly and the prices are unreal.", rating: 5 },
              { name: "Jade M.", text: "My go-to for going out outfits. The dresses are always on trend.", rating: 5 },
              { name: "Sophie R.", text: "Student discount is a game changer. I shop here every week now!", rating: 5 },
            ].map((r, i) => (
              <div key={i} className="bg-[#fdf0f3] rounded-[24px] p-8 border-2 border-[#f4a2b6]/20">
                <div className="flex mb-4">
                  {Array.from({ length: r.rating }).map((_, s) => (
                    <Star key={s} className="w-5 h-5 fill-[#f4a2b6] text-[#f4a2b6]" />
                  ))}
                </div>
                <p className="text-[15px] font-bold text-black mb-4 leading-relaxed">&ldquo;{r.review || r.text}&rdquo;</p>
        ...
        <section className="px-4 md:px-8 max-w-[1600px] mx-auto mb-20 md:mb-24">
          <h2 className="text-[36px] md:text-[48px] font-black uppercase tracking-tighter text-black leading-none mb-8 text-center">
            <Instagram className="w-8 h-8 inline text-[#f4a2b6] mr-2" /> Style Inspo
          </h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2 md:gap-3">
            {[531, 532, 533, 534, 535, 536].map((seed) => (
              <div key={seed} className="relative aspect-square overflow-hidden rounded-[16px] group cursor-pointer">
                <Image src={`https://picsum.photos/400/400?random=${seed}`} alt="Style" fill className="object-cover group-hover:scale-110 transition-transform duration-700" />
                <div className="absolute inset-0 bg-[#f4a2b6]/0 group-hover:bg-[#f4a2b6]/30 transition-colors duration-300 flex items-center justify-center">
        ...
        {/* ─── REWARDS PROGRAM ─── */}
        <section className="px-4 md:px-8 max-w-[1600px] mx-auto mb-20 md:mb-24">
          <div className="bg-gradient-to-r from-[#f4a2b6] to-[#e88da3] rounded-[40px] p-10 md:p-16 text-center text-black shadow-xl">
            <h2 className="text-[40px] md:text-[56px] font-black uppercase tracking-tighter leading-none mb-4">PinkGlam Rewards</h2>
            <p className="text-[18px] md:text-[22px] font-bold mb-8 opacity-80">Earn points on every purchase. Unlock exclusive perks and birthday treats.</p>
            <button className="bg-black text-white px-12 py-4 rounded-full font-black uppercase tracking-widest text-[15px] hover:bg-white hover:text-black transition-all shadow-lg">
              Join Free
            </button>
          </div>
        </section>
        ...
        <section className="px-4 md:px-8 max-w-[800px] mx-auto mb-20 text-center">
          <h2 className="text-[32px] md:text-[44px] font-black uppercase tracking-tighter text-black leading-none mb-4">Get 15% Off</h2>
          <p className="text-[16px] font-bold text-gray-500 mb-8">Sign up for emails and get 15% off your first order</p>
          <div className="flex flex-col sm:flex-row gap-3">
            <input type="email" placeholder="Enter your email" className="flex-1 bg-white border-2 border-black px-6 py-4 font-bold text-[14px] rounded-full outline-none focus:border-[#f4a2b6] transition-colors" />
            <button className="bg-[#f4a2b6] text-black px-10 py-4 font-black uppercase tracking-widest text-[14px] hover:bg-black hover:text-white transition-colors rounded-full shadow-lg">
              Sign Up
            </button>
          </div>
        </section>
