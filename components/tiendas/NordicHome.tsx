  ...
  return (
    <div className="min-h-full bg-white font-sans text-[#111111] overflow-x-hidden selection:bg-[#0058a3] selection:text-white pb-10">
        ...
        <div className="flex items-center space-x-2 md:space-x-4">
          ...
          >
            <div className="relative">
              ...
              {itemCount > 0 && (
                <span className="absolute -top-1.5 -right-2 md:top-[2px] md:-right-2.5 bg-[#0058a3] text-white text-[10px] md:text-[11px] font-bold w-[18px] h-[18px] md:w-[20px] md:h-[20px] flex items-center justify-center rounded-full leading-none border-2 border-white">{itemCount}</span>
              )}
            </div>
      ...
      <main className="w-full max-w-[1600px] mx-auto px-4 lg:px-8 py-8 md:py-12">
                ...
                <div key={product.id || idx} data-product-id={product.id} className="group cursor-pointer flex flex-col relative h-full">
                  ...
                  <div className="flex-1 flex flex-col px-1">
                    ...
                    <div className="flex items-start mb-2 mt-auto">
                      <span className="text-[13px] font-bold mt-[2px] md:mt-[4px] mr-1">Gs.</span>
                      <span className="text-[28px] md:text-[32px] font-black leading-none tracking-tighter">{Math.floor(product.price).toLocaleString('es-PY')}</span>
                    </div>
                    <div className="flex items-center space-x-1.5 mt-2">
                      ...
                      <span className="text-[12px] text-gray-500 font-medium">({product.reviews || 100})</span>
                    </div>
                  </div>
                </div>
              )
        ...
        <div className="bg-[#fbd914] rounded-[8px] md:rounded-[16px] p-6 md:p-12 mb-16 md:mb-24 flex flex-col lg:flex-row items-center gap-8 lg:gap-16 border-2 border-[#f2ca00]">
          ...
          <div className="grid grid-cols-2 gap-4 md:gap-6 w-full lg:w-[55%]">
            {data.products.slice(0, 2).map((product, i) => (
              <div key={`nlp-${i}`} data-product-id={product.id} data-discount={product.discount || 0} className="bg-white rounded-[4px] md:rounded-[8px] p-4 md:p-6 shadow-md hover:shadow-xl transition-shadow cursor-pointer relative group">
                <div className="absolute top-4 left-4 right-4 flex justify-between items-start z-10">
                  {product.discount && product.discount > 0 && (
                    <div className="bg-[#e0001b] text-white text-[10px] md:text-[12px] font-bold px-2 py-1 shadow-sm font-sans uppercase">-{product.discount}%</div>
                  )}
                </div>
                ...
                <div className="flex flex-col space-y-1">
                  <div className="flex items-baseline">
                    <span className="text-[14px] font-bold text-[#111] mr-0.5">$</span>
                    <span className="text-[28px] md:text-[36px] font-black text-[#111] leading-none tracking-tighter">{product.price}</span>
                  </div>
                  {product.originalPrice && (
                    <span className="text-[12px] text-gray-500 line-through font-medium tracking-wide">Previous price: ${product.originalPrice}</span>
                  )}
                </div>
              </div>
            ))}
        ...
        {/* ─── IDEAS & INSPIRATION CAROUSEL ─── */}
        <div className="mb-16 md:mb-24">
          <h3 className="text-[24px] md:text-[28px] font-bold mb-8 tracking-tight">Ideas & Inspiration</h3>
          <div className="flex overflow-x-auto gap-4 md:gap-6 pb-4 snap-x snap-mandatory hide-scrollbar -mx-4 px-4 md:mx-0 md:px-0">
            {[
              { title: 'Small space living', img: 'https://picsum.photos/500/400?random=81' },
              { title: 'Cozy bedroom ideas', img: 'https://picsum.photos/500/400?random=82' },
              { title: 'Kitchen organization', img: 'https://picsum.photos/500/400?random=83' },
              { title: 'Home office setup', img: 'https://picsum.photos/500/400?random=84' },
              { title: 'Outdoor living', img: 'https://picsum.photos/500/400?random=85' },
            ].map((idea, i) => (
              <div key={i} className="group cursor-pointer shrink-0 w-[280px] md:w-[320px] snap-start">
                <div className="relative aspect-[4/3] rounded-[4px] overflow-hidden mb-3">
                  <Image src={idea.img} alt={idea.title} fill className="object-cover group-hover:scale-105 transition-transform duration-700" referrerPolicy="no-referrer" />
                </div>
                <h4 className="font-bold text-[16px] group-hover:underline">{idea.title}</h4>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* ─── RECENTLY VIEWED ─── */}
        <div className="mb-16 md:mb-24">
          <h3 className="text-[24px] md:text-[28px] font-bold mb-8 tracking-tight">Recently Viewed</h3>
          <div className="flex overflow-x-auto gap-4 pb-4 snap-x snap-mandatory hide-scrollbar -mx-4 px-4 md:mx-0 md:px-0">
            {data.products.slice(0, 8).map((product, idx) => (
              <div key={`nh-recent-${idx}`} data-product-id={product.id} className="group cursor-pointer shrink-0 w-[180px] snap-start">
                <div className="relative aspect-square bg-[#f5f5f5] rounded-[4px] overflow-hidden mb-2">
                ...
                <h4 className="text-[13px] font-bold uppercase truncate">{product.title.split(' ')[0]}</h4>
        ...
        {/* ─── NordicHome FOR BUSINESS ─── */}
        <div className="mb-16 md:mb-24 bg-[#111] text-white rounded-[8px] p-8 md:p-12 flex flex-col md:flex-row items-center gap-8">
          <div className="md:w-1/2">
            <h3 className="text-[28px] md:text-[36px] font-black mb-4 tracking-tight">NordicHome for Business</h3>
            <p className="text-[16px] text-gray-300 mb-6 font-medium">Furnish your workspace with smart solutions. Special pricing for businesses.</p>
            <button className="bg-white text-[#111] font-bold text-[14px] py-3 px-8 rounded-full hover:bg-gray-200 transition-colors">Learn more</button>
          </div>
          <div className="md:w-1/2 relative aspect-[16/9] rounded-[4px] overflow-hidden">
            <Image src="https://picsum.photos/800/450?random=86" alt="Business" fill className="object-cover" referrerPolicy="no-referrer" />
          </div>
        </div>
