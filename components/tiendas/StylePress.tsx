        ...
        <div className="bg-[#2d2d2d] w-full h-[60px] flex items-center justify-between px-4 lg:px-8">
          ...
          <div className="flex items-center space-x-5 lg:space-x-7 text-white h-full">
            ...
            >
              ...
              {itemCount > 0 && (
                <span className="absolute -top-1 -right-2 bg-[#d01345] text-white text-[10px] font-black px-1.5 rounded-full">{itemCount}</span>
              )}
            </button>
          </div>
        </div>
      ...
      <main className="w-full max-w-[1440px] mx-auto pb-16">
        ...
        <div className="w-full px-4 md:px-8 mb-24">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-[22px] md:text-[28px] font-black uppercase tracking-tighter">Trending Brands</h3>
            <a href="#" className="text-[13px] font-bold text-[#0088cc] hover:underline uppercase tracking-widest">See All</a>
          </div>
          <div className="flex overflow-x-auto gap-4 pb-4 snap-x hide-scrollbar">
            {[
              { name: 'Topshop', img: 'https://picsum.photos/300/400?random=180' },
              { name: 'COLLUSION', img: 'https://picsum.photos/300/400?random=181' },
              { name: 'Reclaimed Vintage', img: 'https://picsum.photos/300/400?random=182' },
              { name: 'ASOS DESIGN', img: 'https://picsum.photos/300/400?random=183' },
              { name: 'Nike', img: 'https://picsum.photos/300/400?random=184' },
              { name: 'adidas', img: 'https://picsum.photos/300/400?random=185' },
            ].map((brand, i) => (
              <div key={i} className="shrink-0 w-[180px] md:w-[220px] snap-start group cursor-pointer">
                <div className="relative aspect-[3/4] mb-3 overflow-hidden bg-[#f8f8f8]">
                  <Image src={brand.img} alt={brand.name} fill className="object-cover group-hover:scale-[1.03] transition-transform duration-500" referrerPolicy="no-referrer" />
                </div>
                <h4 className="text-[14px] font-bold text-center group-hover:underline">{brand.name}</h4>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="w-full px-4 md:px-8 mb-24">
          <h3 className="text-[22px] md:text-[28px] font-black uppercase tracking-tighter mb-8 text-center">Style Inspiration</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: 'Festival Ready', img: 'https://picsum.photos/400/500?random=186' },
              { label: 'Office Chic', img: 'https://picsum.photos/400/500?random=187' },
              { label: 'Weekend Vibes', img: 'https://picsum.photos/400/500?random=188' },
              { label: 'Date Night', img: 'https://picsum.photos/400/500?random=189' },
            ].map((look, i) => (
              <div key={i} className="relative aspect-[3/4] group cursor-pointer overflow-hidden">
                <Image src={look.img} alt={look.label} fill className="object-cover group-hover:scale-[1.05] transition-transform duration-700" referrerPolicy="no-referrer" />
                <div className="absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-black/70 to-transparent" />
                <div className="absolute bottom-4 left-0 right-0 text-center">
                  <span className="bg-white text-black px-4 py-2 font-bold uppercase text-[12px] tracking-widest">{look.label}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="w-full px-4 md:px-8 mb-24">
          <h3 className="text-[22px] md:text-[28px] font-black uppercase tracking-tighter mb-8">You May Also Like</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {data.products.slice(0, 4).map((product, idx) => (
              <div key={`also-${idx}`} className="group cursor-pointer">
                <div className="relative aspect-[3/4] mb-3 bg-[#f8f8f8] overflow-hidden">
                  <Image src={product.imageUrl} alt={product.title} fill className="object-cover group-hover:scale-[1.02] transition-transform duration-500" referrerPolicy="no-referrer" />
                </div>
                <div className="flex items-center space-x-2 mb-1">
                  <span className="font-bold text-[14px]">${product.price}</span>
                  {product.originalPrice && <span className="text-[12px] text-gray-500 line-through">${product.originalPrice}</span>}
                </div>
                <h4 className="text-[13px] text-[#666] line-clamp-1 group-hover:underline">{product.title}</h4>
              </div>
            ))}
          </div>
        </div>
