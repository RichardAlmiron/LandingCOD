  ...
  return (
    <div className="min-h-full bg-white font-serif text-[#1a1a1a] overflow-x-hidden selection:bg-[#bc001b] selection:text-white" style={{ fontFamily: "'Brilliant Cut Pro', 'Times New Roman', Times, serif" }}>
        ...
        <div className="w-full mx-auto px-4 md:px-8 h-[60px] md:h-[80px] flex items-center justify-between">
          ...
          <div className="flex items-center justify-end space-x-5 md:space-x-6 text-[#1a1a1a] w-1/3">
            ...
            >
              ...
              {itemCount > 0 && (
                <span className="absolute -top-2 -right-2 bg-[#bc001b] text-white text-[8px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{itemCount}</span>
              )}
            </button>
          </div>
        </div>
      ...
      <main className="w-full">
        ...
        {/* ─── FEATURED COLLECTION (Dual Image Split) ─── */}
        <div className="w-full max-w-[1920px] mx-auto py-24 md:py-32 px-4 md:px-10">
          <div className="text-center mb-16 md:mb-20">
            <h2 className="text-[10px] font-sans font-bold uppercase tracking-[0.3em] text-[#c1a673] mb-4">Exclusive Selection</h2>
            <h3 className="text-[28px] md:text-[36px] font-serif text-[#1a1a1a] italic tracking-wide">Featured Collection</h3>
            <div className="w-16 h-[1px] bg-[#c1a673] mx-auto mt-6"></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
            {[
              { title: "Trinity Collection", subtitle: "Three golds, one icon", img: "https://picsum.photos/900/600?random=195" },
              { title: "Panthère de MaisonElegance", subtitle: "Wild elegance since 1914", img: "https://picsum.photos/900/600?random=196" },
            ].map((item, idx) => (
              <div key={idx} className="relative aspect-[3/2] overflow-hidden group cursor-pointer">
                <Image src={item.img} alt={item.title} fill className="object-cover group-hover:scale-[1.03] transition-transform duration-[3000ms]" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
                <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12 text-white">
                  <p className="text-[9px] font-sans font-bold uppercase tracking-[0.25em] text-[#c1a673] mb-2">{item.subtitle}</p>
                  <h4 className="text-[24px] md:text-[30px] font-serif italic mb-4">{item.title}</h4>
                  <span className="text-[9px] font-sans font-bold uppercase tracking-[0.2em] border-b border-white pb-1 group-hover:text-[#c1a673] group-hover:border-[#c1a673] transition-colors">
                    Discover <ChevronRight className="w-3 h-3 inline ml-1" strokeWidth={2} />
                  </span>
                </div>
              </div>
            ))}
        ...
        {/* ─── BRAND HERITAGE (Timeline Style) ─── */}
        <div className="w-full bg-[#f5f0eb] py-24 md:py-32 px-4 md:px-10 border-t border-[#e5ddd3]">
          <div className="max-w-[1920px] mx-auto">
            <div className="text-center mb-16 md:mb-20">
              <h2 className="text-[10px] font-sans font-bold uppercase tracking-[0.3em] text-[#c1a673] mb-4">Since 1847</h2>
              <h3 className="text-[28px] md:text-[36px] font-serif text-[#1a1a1a] italic tracking-wide">Our Heritage</h3>
              <div className="w-16 h-[1px] bg-[#c1a673] mx-auto mt-6"></div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8 md:gap-6">
              {[
                { year: "1847", title: "The Beginning", desc: "Louis-François MaisonElegance opens his first jewelry workshop in Paris." },
                { year: "1904", title: "Santos Watch", desc: "The first modern wristwatch, created for aviator Alberto Santos-Dumont." },
                { year: "1914", title: "The Panther", desc: "The iconic panther motif makes its first appearance in MaisonElegance creations." },
                { year: "1969", title: "Love Bracelet", desc: "Aldo Cipullo designs the legendary Love bracelet in New York." },
              ].map((milestone, idx) => (
                <div key={idx} className="flex flex-col items-center text-center group cursor-pointer">
                  <div className="w-16 h-16 rounded-full border-2 border-[#c1a673] flex items-center justify-center mb-6 group-hover:bg-[#bc001b] group-hover:border-[#bc001b] transition-all duration-500">
                    <span className="text-[14px] font-serif italic text-[#1a1a1a] group-hover:text-white transition-colors">{milestone.year}</span>
                  </div>
                  <h4 className="text-[16px] font-serif text-[#1a1a1a] mb-3 italic">{milestone.title}</h4>
                  <p className="text-[12px] font-sans font-light text-[#666] leading-relaxed max-w-[250px]">{milestone.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="w-full py-24 md:py-32 px-4 md:px-10">
          <div className="max-w-[1920px] mx-auto">
            <div className="text-center mb-16 md:mb-20">
              <h2 className="text-[10px] font-sans font-bold uppercase tracking-[0.3em] text-[#c1a673] mb-4">@MaisonElegance</h2>
              <h3 className="text-[28px] md:text-[36px] font-serif text-[#1a1a1a] italic tracking-wide">Follow the Maison</h3>
              <div className="w-16 h-[1px] bg-[#c1a673] mx-auto mt-6"></div>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2 md:gap-3">
              {[197, 198, 199, 200, 201, 202].map((seed) => (
                <div key={seed} className="relative aspect-square overflow-hidden group cursor-pointer">
                  <Image src={`https://picsum.photos/400/400?random=${seed}`} alt="Instagram" fill className="object-cover group-hover:scale-110 transition-transform duration-700" />
                  <div className="absolute inset-0 bg-[#bc001b]/0 group-hover:bg-[#bc001b]/20 transition-colors duration-300 flex items-center justify-center">
                    <Heart className="w-8 h-8 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300" strokeWidth={1} />
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* ─── NEWSLETTER SECTION ─── */}
        <div className="w-full bg-[#1a1a1a] py-20 md:py-28 px-4 md:px-10">
          <div className="max-w-[700px] mx-auto text-center">
            <h2 className="text-[10px] font-sans font-bold uppercase tracking-[0.3em] text-[#c1a673] mb-4">Stay Informed</h2>
            <h3 className="text-[28px] md:text-[36px] font-serif text-white italic tracking-wide mb-6">Maison Newsletter</h3>
            <p className="text-[13px] font-sans font-light text-[#999] leading-relaxed mb-10">
              Be the first to discover our latest creations, exclusive events, and the stories behind the Maison.
            </p>
            <div className="flex flex-col sm:flex-row gap-3 max-w-[500px] mx-auto">
              <input
                type="email"
                placeholder="Your email address"
                className="flex-1 bg-transparent border-b border-[#555] text-white placeholder-[#777] py-3 px-1 text-[12px] font-sans outline-none focus:border-[#c1a673] transition-colors"
              />
              <button className="bg-[#bc001b] text-white px-8 py-3 font-sans font-bold uppercase text-[10px] tracking-[0.2em] hover:bg-[#a00018] transition-colors">
                Subscribe
              </button>
            </div>
          </div>
