      ...
      <header className={`fixed top-0 inset-x-0 z-50 transition-colors duration-300 ${isScrolled ? 'bg-white shadow-sm' : 'bg-transparent'}`}>
        <div className="w-full px-6 md:px-12 h-[56px] flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2 md:space-x-1">
            ...
            <button onClick={() => setIsCartOpen(true)} className={`p-1.5 rounded-[4px] transition-colors ${isScrolled ? 'hover:bg-gray-100 text-black' : 'hover:bg-white/10 text-white'}`}>
      ...
      {activeTab === 'vehicles' ? (
        ...
        <main className="pt-[80px] min-h-screen bg-gray-50">
                ...
                {paginatedItems.map((product, idx) => (
                  ...
                  >
                    ...
                    <button 
                      ...
                      className="w-full mt-3 bg-black text-white py-2 rounded font-medium text-sm hover:bg-gray-800 transition-colors opacity-0 group-hover:opacity-100"
                    >
                      Add to Cart
                    </button>
                  </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm mb-8">
            <h2 className="text-2xl font-bold mb-6">Trending Accessories</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { title: 'All-Weather Floor Mats', img: 'https://picsum.photos/400/400?random=501', price: '225' },
                { title: 'Wall Connector', img: 'https://picsum.photos/400/400?random=502', price: '475' },
                { title: 'Roof Rack', img: 'https://picsum.photos/400/400?random=503', price: '450' },
                { title: 'Center Console Trays', img: 'https://picsum.photos/400/400?random=504', price: '35' },
              ].map((item, i) => (
                <div key={i} className="group cursor-pointer bg-gray-50 rounded-lg p-4 hover:shadow-md transition-shadow">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { name: 'Michael S.', text: 'The wall connector installation was seamless. Charges my car overnight with no issues.', rating: 5 },
                { name: 'Jennifer L.', text: 'Floor mats fit perfectly and are super easy to clean. Great quality for the price.', rating: 5 },
                { name: 'Robert K.', text: 'Roof rack is solid and aerodynamic. No noticeable impact on range during highway driving.', rating: 4 },
              ].map((review, i) => (
          ...
          <div className="bg-white rounded-xl overflow-hidden shadow-sm mb-8">
            <div className="flex flex-col md:flex-row">
              <div className="w-full md:w-1/2 relative h-[300px] md:h-auto">
                <Image src="https://picsum.photos/800/500?random=510" alt="Lifestyle" fill className="object-cover" />
              </div>
              <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
                <h2 className="text-3xl font-bold mb-4">Drive Into The Future</h2>
                <p className="text-gray-600 mb-6 leading-relaxed">
                  Experience the next generation of electric mobility. From autonomous driving to over-the-air updates, every journey is an upgrade.
                </p>
                <button className="bg-black text-white px-6 py-3 rounded-[4px] font-medium hover:bg-gray-800 transition-colors w-fit">
                  Explore Features
                </button>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              { icon: <Truck className="w-8 h-8 text-blue-600" />, title: 'Free Shipping', desc: 'On all orders over $50' },
              { icon: <Shield className="w-8 h-8 text-green-600" />, title: '2-Year Warranty', desc: 'Full coverage included' },
              { icon: <RotateCcw className="w-8 h-8 text-orange-600" />, title: '30-Day Returns', desc: 'Hassle-free process' },
              { icon: <Zap className="w-8 h-8 text-yellow-600" />, title: 'Fast Delivery', desc: '2-3 business days' },
            ].map((s, i) => (
          ...
          {/* Newsletter */}
          <div className="bg-black text-white rounded-xl p-8 md:p-12 mb-8 text-center">
            <h2 className="text-3xl font-bold mb-4">Stay Updated</h2>
            <p className="text-gray-400 mb-8 max-w-lg mx-auto">Get the latest on new products, software updates, and exclusive offers delivered to your inbox.</p>
            <div className="flex max-w-md mx-auto">
              <input type="email" placeholder="Email address" className="bg-white/10 border border-white/20 text-white px-4 py-3 rounded-l-[4px] outline-none flex-1 placeholder-gray-500" />
              <button className="bg-white text-black px-6 py-3 rounded-r-[4px] font-medium hover:bg-gray-200 transition-colors shrink-0">Subscribe</button>
            </div>
          </div>

          {/* Sustainability */}
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-8 md:p-12 mb-8 flex flex-col md:flex-row items-center gap-8">
            <div className="w-full md:w-1/2">
              <h2 className="text-3xl font-bold mb-4">Our Sustainability Mission</h2>
              <p className="text-gray-600 mb-6 leading-relaxed">
                Accelerating the world&apos;s transition to sustainable energy. Every product we make is designed with the planet in mind.
              </p>
              <button className="bg-black text-white px-6 py-3 rounded-[4px] font-medium hover:bg-gray-800 transition-colors">
                Learn More
              </button>
            </div>
            <div className="w-full md:w-1/2 relative h-[250px] rounded-lg overflow-hidden">
              <Image src="https://picsum.photos/800/500?random=515" alt="Sustainability" fill className="object-cover" />
            </div>
          </div>
          </div>
