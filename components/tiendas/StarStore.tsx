...
import { Search, ShoppingBag, Heart, User, Menu, Star, ChevronDown, MapPin, ArrowRight, Gift, CreditCard, Smartphone, Facebook, Twitter, Instagram, Youtube, Percent, Clock, Box, ShieldCheck, Sparkles, X, Truck } from 'lucide-react';
          ...
          <div className="py-5 flex items-center justify-between gap-10">
            ...
            <div className="flex items-center gap-8 text-stone-600">
              ...
              >
                ...
                <span className="bg-[#e01a2b] text-white text-[10px] font-black px-1.5 py-0.5 rounded-full absolute -top-1 -right-1 ring-2 ring-white shadow-sm">{itemCount}</span>
      ...
      <main className="max-w-[1440px] mx-auto px-6 py-10">
        ...
        <div className="mb-32">
          <div className="text-center mb-16">
            <div className="text-[12px] font-black tracking-[0.4em] text-stone-300 mb-2 uppercase">Explore</div>
            <h2 className="text-5xl font-black uppercase tracking-tighter italic">Shop by <span className="text-[#e01a2b]">Department</span></h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            {[
              { name: 'Women', img: 'https://picsum.photos/300/400?random=410' },
              { name: 'Men', img: 'https://picsum.photos/300/400?random=411' },
              { name: 'Kids', img: 'https://picsum.photos/300/400?random=412' },
              { name: 'Home', img: 'https://picsum.photos/300/400?random=413' },
              { name: 'Beauty', img: 'https://picsum.photos/300/400?random=414' },
              { name: 'Shoes', img: 'https://picsum.photos/300/400?random=415' },
            ].map((dept, i) => (
              <div key={i} className="group cursor-pointer text-center">
                <div className="relative aspect-square overflow-hidden rounded-full mb-4 border-4 border-transparent group-hover:border-[#e01a2b] transition-all">
                ...
                <h4 className="font-black text-sm uppercase tracking-widest group-hover:text-[#e01a2b] transition-colors">{dept.name}</h4>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Trending Brands */}
        <div className="mb-32 bg-stone-50 rounded-3xl p-12 lg:p-20 border border-stone-200">
          <div className="flex items-end justify-between mb-12">
            <div>
              <div className="text-[12px] font-black tracking-[0.4em] text-stone-300 mb-2 uppercase">Featured</div>
              <h2 className="text-4xl font-black uppercase tracking-tighter italic">Trending <span className="text-[#e01a2b]">Brands</span></h2>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {data.products.slice(0, 4).map((product, i) => (
              <div key={`brand-${i}`} className="group cursor-pointer bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all">
                <div className="relative aspect-square overflow-hidden">
                  ...
                  <button onClick={(e) => handleAddToCart(product, e)} className="absolute bottom-4 left-4 right-4 bg-[#e01a2b] text-white py-3 font-black uppercase text-[11px] tracking-widest opacity-0 group-hover:opacity-100 transition-opacity shadow-xl">Quick Add</button>
                </div>
                <div className="p-5">
                  <h4 className="font-black text-sm uppercase tracking-tight line-clamp-1">{product.title}</h4>
                  <span className="text-[#e01a2b] font-black text-lg">${product.price}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mb-32">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-black uppercase tracking-tighter italic">What Our <span className="text-[#e01a2b]">Customers</span> Say</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { name: 'Jennifer R.', text: 'Star Rewards is the best loyalty program. I earned $50 in Star Money last month!', rating: 5 },
              { name: 'Michael T.', text: 'Amazing selection and the sales are unbeatable. My go-to department store.', rating: 5 },
              { name: 'Lisa P.', text: 'Love the beauty section. Free gifts with purchase are always a nice surprise.', rating: 4 },
            ].map((review, i) => (
              <div key={i} className="bg-stone-50 rounded-2xl p-8 border border-stone-200 hover:shadow-lg transition-shadow">
                <div className="flex mb-4">{Array.from({ length: review.rating }).map((_, j) => <Star key={j} className="w-4 h-4 fill-amber-400 text-amber-400" />)}</div>
                <p className="text-sm font-semibold text-stone-700 mb-6 leading-relaxed">"{review.text}"</p>
                <p className="text-xs font-black uppercase tracking-widest text-[#e01a2b]">— {review.name}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Services & Benefits */}
        <div className="mb-32 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            { icon: <Truck className="w-8 h-8" />, title: 'Free Shipping', desc: 'On orders $25+' },
            { icon: <ShieldCheck className="w-8 h-8" />, title: 'Easy Returns', desc: 'Free returns within 90 days' },
            { icon: <Gift className="w-8 h-8" />, title: 'Gift Services', desc: 'Free gift wrapping available' },
            { icon: <CreditCard className="w-8 h-8" />, title: 'Star Card', desc: 'Extra 20% off with Star Card' },
          ].map((service, i) => (
            <div key={i} className="flex flex-col items-center text-center p-8 border border-stone-200 rounded-2xl hover:border-[#e01a2b] transition-colors group">
              <div className="text-stone-400 group-hover:text-[#e01a2b] transition-colors mb-4">{service.icon}</div>
              <h4 className="font-black text-sm uppercase tracking-widest mb-2">{service.title}</h4>
              <p className="text-xs text-stone-500 font-medium">{service.desc}</p>
            </div>
          ))}
        </div>

        {/* App Download Banner */}
        <div className="mb-32 bg-[#e01a2b] text-white rounded-3xl p-12 lg:p-20 flex flex-col md:flex-row items-center justify-between">
          <div className="mb-8 md:mb-0">
            <h2 className="text-4xl font-black uppercase tracking-tighter italic mb-4">Get the StarStore App</h2>
            <p className="text-lg font-medium opacity-90 max-w-md">Shop smarter with exclusive app-only deals, scan & pay in store, and manage your Star Rewards on the go.</p>
          </div>
          <div className="flex gap-4">
            <button className="bg-white text-[#e01a2b] px-8 py-4 font-black uppercase text-xs tracking-widest hover:bg-stone-100 transition-colors shadow-xl rounded-lg">App Store</button>
            <button className="bg-white text-[#e01a2b] px-8 py-4 font-black uppercase text-xs tracking-widest hover:bg-stone-100 transition-colors shadow-xl rounded-lg">Google Play</button>
          </div>
        </div>
