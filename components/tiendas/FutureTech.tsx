    ...
    <div className="min-h-full bg-white font-sans text-black selection:bg-black selection:text-white pb-10 overflow-x-hidden">
        ...
        <div className="w-full max-w-[1440px] mx-auto px-4 lg:px-6 h-[64px] flex items-center justify-between">
          ...
          <div className="flex items-center h-full space-x-2 md:space-x-4">
            ...
            >
              ...
              {itemCount > 0 && (
                <div className="absolute top-1 right-1 w-4 h-4 bg-[#2189ff] text-white text-[10px] font-bold rounded-full border border-white flex items-center justify-center">{itemCount}</div>
              )}
            </button>
      ...
      <main>
        ...
        <div className="w-full max-w-[1440px] mx-auto px-4 lg:px-8 py-16 md:py-24">
          ...
          {/* ─── GALAXY ECOSYSTEM BANNER ─── */}
          <div className="w-full bg-black rounded-[24px] overflow-hidden flex flex-col items-center text-center py-20 px-8 mb-24 relative">
            <div className="absolute inset-0 bg-gradient-to-br from-blue-900/30 via-transparent to-purple-900/20" />
            <div className="relative z-10">
              <span className="text-[#2189ff] font-bold text-[13px] tracking-widest uppercase mb-4 block">Galaxy Ecosystem</span>
              <h2 className="text-[36px] md:text-[52px] font-bold text-white tracking-tight mb-6 leading-[1.1]">Better Together</h2>
              <p className="text-[16px] md:text-[18px] text-gray-300 mb-10 max-w-2xl mx-auto leading-relaxed">
                Seamlessly connect your Galaxy phone, watch, buds, and tablet. Share files instantly, answer calls from any device, and enjoy a unified experience.
              </p>
              <div className="flex flex-wrap justify-center gap-8 mb-10">
                {['Phone', 'Watch', 'Buds', 'Tablet', 'Laptop'].map((device, i) => (
                  <div key={i} className="flex flex-col items-center gap-2">
                    <div className="w-16 h-16 rounded-full bg-white/10 flex items-center justify-center border border-white/20">
                      <Smartphone className="w-6 h-6 text-white" />
                    </div>
                    <span className="text-white text-[12px] font-bold">{device}</span>
                  </div>
                ))}
              </div>
              <button className="bg-[#2189ff] text-white px-10 py-3.5 rounded-full font-bold text-[15px] hover:bg-blue-500 transition-colors shadow-lg">
                Explore Galaxy
              </button>
            </div>
          </div>
          ...
          {/* ─── CUSTOMER REVIEWS ─── */}
          <div className="mb-24">
            <h2 className="text-[28px] md:text-[36px] font-bold tracking-tight mb-10 text-center">What Customers Say</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { name: 'Alex M.', rating: 5, text: 'The camera quality is absolutely stunning. Best phone I have ever owned.', product: 'Galaxy Ultra' },
                { name: 'Sarah K.', rating: 5, text: 'Battery lasts all day even with heavy use. The ecosystem integration is seamless.', product: 'Galaxy Watch' },
                { name: 'James L.', rating: 4, text: 'Incredible display quality. Perfect for streaming and productivity on the go.', product: 'Galaxy Tab' },
              ].map((review, i) => (
                <div key={i} className="bg-[#f4f4f4] rounded-[16px] p-8 flex flex-col">
                  <div className="flex items-center gap-1 mb-4">
                    {Array.from({ length: 5 }).map((_, s) => (
                      <div key={s} className={`w-4 h-4 rounded-full ${s < review.rating ? 'bg-yellow-400' : 'bg-gray-300'}`} />
                    ))}
                  </div>
                  <p className="text-[15px] text-gray-700 mb-6 flex-1 leading-relaxed italic">"{review.text}"</p>
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-[14px]">{review.name}</span>
                    <span className="text-[12px] text-gray-500 font-medium">{review.product}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* ─── NEWSLETTER SIGNUP ─── */}
          <div className="bg-[#f4f4f4] rounded-[24px] p-10 md:p-16 flex flex-col md:flex-row items-center justify-between gap-8 mb-24">
            <div className="flex-1 text-center md:text-left">
              <h2 className="text-[28px] md:text-[36px] font-bold tracking-tight mb-3">Stay in the Loop</h2>
              <p className="text-[15px] text-gray-600 font-medium">Get exclusive offers, product launches, and tech tips delivered to your inbox.</p>
            </div>
            <div className="flex w-full md:w-auto gap-3">
              <input type="email" placeholder="Enter your email" className="bg-white border border-gray-300 rounded-full px-6 py-3 text-[14px] w-full md:w-72 outline-none focus:border-black transition-colors" />
              <button className="bg-black text-white px-8 py-3 rounded-full font-bold text-[14px] hover:bg-gray-800 transition-colors whitespace-nowrap shadow-sm">
                Sign Up
              </button>
            </div>
          </div>
