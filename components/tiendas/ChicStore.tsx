          ...
          <div className="w-full flex items-center justify-between mb-8">
            ...
            <div className="flex items-center space-x-4 md:space-x-8">
              ...
              >
                ...
                {itemCount > 0 && (
                  <span className="bg-black text-white text-[9px] font-black px-1.5 py-0.5 rounded-full absolute -top-2 -right-2">{itemCount}</span>
                )}
              </div>
            </div>
          </div>
      ...
      <main className="max-w-[1440px] mx-auto px-6">
        ...
        <div className="mb-32">
          <div className="text-center mb-20">
            <div className="text-[11px] font-black tracking-[0.5em] text-zinc-300 mb-4 uppercase">Curated Selection</div>
            <h2 className="text-5xl font-serif uppercase tracking-tight italic">Trending This Week</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {data.products.slice(0, 4).map((product, i) => (
              <div key={`trend-${i}`} className="group cursor-pointer">
                <div className="relative aspect-[3/4] overflow-hidden mb-4 bg-zinc-50">
                  ...
                  <button onClick={(e) => handleAddToCart(product, e)} className="absolute bottom-4 left-4 right-4 bg-black text-white py-3 text-[10px] font-black tracking-widest uppercase opacity-0 group-hover:opacity-100 transition-opacity">Add to Bag</button>
                </div>
                <h3 className="text-xs font-black uppercase tracking-widest line-clamp-1">{product.title}</h3>
                <span className="font-serif italic text-base">${product.price}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mb-32 grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div className="group cursor-pointer relative aspect-[4/5] overflow-hidden">
            <img src="https://picsum.photos/800/1000?random=210" alt="Editorial" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
            <div className="absolute bottom-12 left-12 right-12 text-white">
              <div className="text-[10px] font-black tracking-[0.4em] uppercase mb-4">Spring Lookbook</div>
              <h3 className="text-4xl font-serif italic mb-6">The Art of Dressing Well</h3>
              <button className="bg-white text-black px-10 py-3 font-black text-[11px] uppercase tracking-widest hover:bg-zinc-200 transition-colors">Explore</button>
            </div>
          </div>
          <div className="group cursor-pointer relative aspect-[4/5] overflow-hidden">
            <img src="https://picsum.photos/800/1000?random=211" alt="Editorial" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
            <div className="absolute bottom-12 left-12 right-12 text-white">
              <div className="text-[10px] font-black tracking-[0.4em] uppercase mb-4">Home Collection</div>
              <h3 className="text-4xl font-serif italic mb-6">Luxury Living Essentials</h3>
              <button className="bg-white text-black px-10 py-3 font-black text-[11px] uppercase tracking-widest hover:bg-zinc-200 transition-colors">Shop Now</button>
            </div>
          </div>
        </div>

        {/* Customer Reviews */}
        <div className="mb-32 bg-zinc-50 rounded-[4px] p-12 lg:p-20 border border-zinc-100">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-serif italic tracking-tight">What Our Clients Say</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {[
              { name: 'Alexandra V.', text: 'The Loyallist program is exceptional. Triple points on beauty purchases is a dream.', stars: 5 },
              { name: 'David M.', text: 'Impeccable curation. Every visit feels like a personal shopping experience.', stars: 5 },
              { name: 'Catherine L.', text: 'The designer selection is unmatched. My favorite luxury destination.', stars: 5 },
            ].map((review, i) => (
              <div key={i} className="text-center">
                <div className="flex justify-center mb-4">{Array.from({ length: review.stars }).map((_, j) => <Star key={j} className="w-4 h-4 fill-black text-black" />)}</div>
                <p className="text-sm font-medium text-zinc-600 mb-6 leading-relaxed italic">"{review.text}"</p>
                <p className="text-[10px] font-black uppercase tracking-[0.3em]">{review.name}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mb-32 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            { icon: <Box className="w-6 h-6" />, title: 'Free Shipping', desc: 'On all Loyallist orders' },
            { icon: <Gift className="w-6 h-6" />, title: 'Gift Services', desc: 'Complimentary gift wrapping' },
            { icon: <Phone className="w-6 h-6" />, title: 'Personal Stylist', desc: 'Book a free consultation' },
            { icon: <Globe className="w-6 h-6" />, title: 'Global Delivery', desc: 'Ship to 100+ countries' },
          ].map((service, i) => (
            <div key={i} className="flex flex-col items-center text-center p-8 border border-zinc-100 hover:border-black transition-colors group">
              <div className="text-zinc-400 group-hover:text-black transition-colors mb-4">{service.icon}</div>
              <h4 className="font-black text-[11px] uppercase tracking-[0.2em] mb-2">{service.title}</h4>
              <p className="text-[11px] text-zinc-400 font-medium">{service.desc}</p>
            </div>
          ))}
        </div>

        {/* Newsletter Banner */}
        <div className="mb-32 bg-black text-white rounded-[4px] p-12 lg:p-20 flex flex-col md:flex-row items-center justify-between">
          <div className="mb-8 md:mb-0">
            <h2 className="text-3xl font-serif italic mb-4">Stay in the Know</h2>
            <p className="text-zinc-400 text-sm font-medium max-w-md">Be the first to hear about new arrivals, exclusive events, and Loyallist-only offers.</p>
          </div>
          <div className="flex w-full md:w-auto">
            <input type="email" placeholder="EMAIL ADDRESS" className="bg-white/10 border border-zinc-700 px-6 py-4 text-[11px] font-black tracking-widest outline-none w-full md:w-72 placeholder:text-zinc-500" />
            <button className="bg-white text-black px-8 py-4 font-black text-[11px] uppercase tracking-widest hover:bg-zinc-200 transition-colors">Subscribe</button>
          </div>
        </div>
