  ...
  return (
    <div className="min-h-full bg-white font-sans text-[#222] overflow-x-hidden">
        ...
        <div className="w-full max-w-[1440px] mx-auto px-4 lg:px-6 py-2 md:py-3 flex items-center justify-between gap-3 md:gap-6">
          ...
          <div className="flex items-center gap-1 md:gap-2 shrink-0 text-[#222]">
            ...
            >
              ...
              {itemCount > 0 && (
                <span className="absolute top-1 right-0 bg-[#F1641E] text-white text-[10px] font-bold w-[16px] h-[16px] flex items-center justify-center rounded-full leading-none border border-white">{itemCount}</span>
              )}
            </button>
          </div>
        </div>
      ...
      <main className="w-full max-w-[1440px] mx-auto px-4 lg:px-8 py-6 md:py-8">
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-4 gap-y-8">
            {data.products.map((product, idx) => {
              // Usar product.discount para determinar tags
              const hasDiscount = product.discount && product.discount > 0;
              const hasHighDiscount = product.discount && product.discount > 20;
              let tagText = '';
              let tagColor = '';
              if (hasHighDiscount) {
                tagText = "Bestseller";
              ...
              return (
                <div key={`handcraft-${idx}-${product.id}`} data-product-id={product.id} data-discount={product.discount || 0} className="group cursor-pointer flex flex-col">
                  ...
                  <div className="flex items-center space-x-1 mb-1">
                    ...
                    <span className="text-[12px] text-gray-600">({product.reviews || 0})</span>
                  </div>

                  {/* Price Block */}
                  <div className="flex items-baseline gap-2 mb-1">
                    <span className="text-[16px] font-bold text-[#222] leading-none">${product.price}</span>
                    {product.discount && product.discount > 0 && <span className="text-[13px] text-green-700 bg-green-100 px-1 rounded-sm font-medium">{product.discount}% off</span>}
                  </div>
        ...
        <div className="mb-16">
          <h2 className="text-[24px] font-bold text-[#222] tracking-tight mb-6">Recently Viewed & More</h2>
          <div className="flex overflow-x-auto gap-4 pb-4 snap-x snap-mandatory hide-scrollbar -mx-4 px-4">
            {data.products.slice(0, 8).map((product, idx) => (
              <div key={`hc-recent-${idx}`} data-product-id={product.id} className="group cursor-pointer shrink-0 w-[180px] snap-start">
                <div className="relative aspect-square bg-[#f4f4f4] rounded-[8px] overflow-hidden mb-2">
                  <img src={product.imageUrl} alt={product.title} className="w-full h-full object-cover mix-blend-multiply group-hover:scale-105 transition-transform duration-500" />
                </div>
                <h3 className="text-[13px] text-[#222] line-clamp-1 group-hover:underline font-medium">{product.title}</h3>
        ...
        <div className="mb-16 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="relative rounded-[12px] overflow-hidden h-[280px] group cursor-pointer">
            <img src="https://picsum.photos/800/400?random=186" alt="Gifts" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
            <div className="absolute bottom-6 left-6 text-white">
              <p className="text-[12px] font-bold uppercase tracking-wider mb-2">Gift Guide</p>
              <h3 className="text-[24px] font-serif mb-3">Gifts Under $30</h3>
              <span className="bg-white text-[#222] px-4 py-2 rounded-full text-[13px] font-bold inline-block">Shop now</span>
            </div>
          </div>
          <div className="relative rounded-[12px] overflow-hidden h-[280px] group cursor-pointer">
            <img src="https://picsum.photos/800/400?random=187" alt="Personalized" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
            <div className="absolute bottom-6 left-6 text-white">
              <p className="text-[12px] font-bold uppercase tracking-wider mb-2">Personalized</p>
              <h3 className="text-[24px] font-serif mb-3">Custom & Made-to-Order</h3>
              <span className="bg-white text-[#222] px-4 py-2 rounded-full text-[13px] font-bold inline-block">Explore</span>
            </div>
          </div>
        </div>
        ...
        <div className="mb-16">
          <h2 className="text-[24px] font-bold text-[#222] tracking-tight mb-2">Star Seller Spotlight</h2>
          <p className="text-[15px] text-gray-600 mb-6">Consistently earned 5-star reviews, shipped on time, and replied quickly.</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {data.products.slice(5, 10).map((product, idx) => (
              <div key={`hc-star-${idx}`} data-product-id={product.id} className="group cursor-pointer flex flex-col">
                <div className="relative aspect-square bg-[#f4f4f4] rounded-[8px] overflow-hidden mb-2">
                  <img src={product.imageUrl} alt={product.title} className="w-full h-full object-cover mix-blend-multiply group-hover:scale-105 transition-transform duration-500" />
                  <div className="absolute top-2 left-2 bg-[#fdedc9] text-[#222] text-[10px] font-bold px-2 py-0.5 rounded-[4px] flex items-center gap-1">
                    🌟 Star Seller
                  </div>
                </div>
                <h3 className="text-[13px] text-[#222] line-clamp-1 group-hover:underline font-medium">{product.title}</h3>
                <div className="flex items-center gap-1 mb-0.5">
                  <div className="flex text-[#222]">{[...Array(5)].map((_, i) => <Star key={i} className="w-3 h-3 fill-current" />)}</div>
        ...
        {/* Trending Searches */}
        <div className="mb-16 bg-[#faf9f6] rounded-[16px] p-8">
          <h2 className="text-[24px] font-bold text-[#222] tracking-tight mb-6">Trending Searches</h2>
          <div className="flex flex-wrap gap-3">
            {['Handmade jewelry', 'Vintage clothing', 'Custom portraits', 'Candles', 'Tote bags', 'Stickers', 'Wall art', 'Mugs', 'Phone cases', 'Earrings', 'Rings', 'Planters'].map((term, i) => (
              <button key={i} className="bg-white border border-gray-200 px-4 py-2.5 rounded-full text-[14px] font-medium text-[#222] hover:border-[#222] hover:shadow-sm transition-all cursor-pointer">
                {term}
              </button>
            ))}
          </div>
