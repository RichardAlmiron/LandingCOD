  ...
  return (
    <div className="min-h-full bg-white font-sans text-[#000]">
      ...
      <main className="w-full mx-auto px-6 py-12">
        ...
        <div className="mb-24">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-serif uppercase tracking-widest mb-4">Trending Now</h2>
            <div className="w-16 h-px bg-black mx-auto" />
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {data.products.slice(0, 4).map((product, idx) => (
              <div key={`trend-${product.id || idx}`} className="group cursor-pointer">
                <div className="relative aspect-[3/4] mb-4 overflow-hidden bg-[#f9f9f9]">
                  <img src={product.imageUrl} alt={product.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" />
                  <button onClick={(e) => { e.stopPropagation(); toggleFavorite(product.id); }} className="absolute bottom-3 right-3 bg-white p-2 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity">
                    <Heart className={`w-4 h-4 ${favorites.includes(product.id) ? 'fill-red-500 text-red-500' : ''}`} />
                  </button>
                  <button onClick={(e) => handleAddToCart(product, e)} className="absolute bottom-3 left-3 bg-black text-white px-3 py-1.5 text-[10px] uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">Add</button>
                  {idx === 0 && <div className="absolute top-3 left-3 bg-black text-white text-[9px] font-bold uppercase tracking-widest px-2 py-1">Trending</div>}
                </div>
                <div className="text-center">
                  <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-gray-400">{product.category}</div>
                  <h3 className="text-[11px] font-bold uppercase tracking-widest line-clamp-1 group-hover:underline">{product.title}</h3>
        ...
        <div className="mb-24">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-serif uppercase tracking-widest mb-4">Style Guide</h2>
            <div className="w-16 h-px bg-black mx-auto" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { title: 'Resort Dressing', img: 'https://picsum.photos/500/400?random=446' },
              { title: 'Office to Evening', img: 'https://picsum.photos/500/400?random=447' },
              { title: 'Weekend Casual', img: 'https://picsum.photos/500/400?random=448' },
            ].map((guide, i) => (
              <div key={i} className="group cursor-pointer">
                <div className="relative aspect-[5/4] overflow-hidden mb-4">
                  <img src={guide.img} alt={guide.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" />
                </div>
                <h3 className="text-lg font-serif uppercase tracking-widest group-hover:underline underline-offset-4">{guide.title}</h3>
                <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mt-2 inline-block">Read More →</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Brand Values */}
        <div className="mb-24 border-t border-b border-gray-200 py-16">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {[
              { icon: '✈️', title: 'Global Shipping', desc: 'To 190+ countries' },
              { icon: '↩️', title: 'Free Returns', desc: 'Within 28 days' },
              { icon: '🔒', title: 'Secure Payment', desc: 'SSL encrypted' },
              { icon: '💎', title: '100% Authentic', desc: 'Guaranteed genuine' },
            ].map((v, i) => (
              <div key={i} className="p-4">
                <div className="text-2xl mb-3">{v.icon}</div>
                <h4 className="text-[11px] font-bold uppercase tracking-widest mb-1">{v.title}</h4>
        ...
        <div className="mb-24 text-center">
          <h2 className="text-3xl font-serif uppercase tracking-widest mb-4">Join the LuxEdit World</h2>
          <div className="w-16 h-px bg-black mx-auto mb-8" />
          <p className="text-sm text-gray-500 mb-8 max-w-md mx-auto">Sign up for 10% off your first order plus exclusive access to new arrivals and sales.</p>
          <form className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <input type="email" placeholder="Email Address" className="flex-1 border-b border-black pb-2 bg-transparent outline-none text-[12px] font-bold tracking-widest placeholder-gray-400 focus:border-gray-500 transition-colors" />
            <button type="button" className="text-black font-bold text-[11px] uppercase tracking-[0.2em] hover:text-gray-500 transition-colors">Subscribe</button>
          </form>
        </div>
