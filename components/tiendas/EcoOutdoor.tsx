  ...
  return (
    <div className="min-h-full bg-[#f6f6f6] font-sans text-[#1a1a1a] selection:bg-[#1a1a1a] selection:text-white pb-0 overflow-x-hidden">
        ...
        <div className="w-full mx-auto px-4 md:px-8 h-full flex items-center justify-between">
          ...
          <div className="flex items-center space-x-6 text-[#1a1a1a]">
            ...
            >
              ...
              {itemCount > 0 && (
                <span className="absolute -top-1 -right-2 bg-[#1a1a1a] text-white text-[10px] font-bold px-1.5 rounded-full min-w-[16px] text-center border-2 border-white">{itemCount}</span>
              )}
            </div>
          </div>
        </div>
      ...
      <main className="w-full">
        ...
        {/* ─── SHOP BY CATEGORY ─── */}
        <section className="px-4 md:px-8 max-w-[1600px] mx-auto mb-20">
          <div className="flex items-baseline justify-between mb-10 border-b border-gray-300 pb-4">
            <h2 className="text-[32px] md:text-[40px] font-black uppercase tracking-tighter text-[#1a1a1a]">Shop by Activity</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { name: 'Climbing', img: 'https://picsum.photos/500/600?random=330' },
              { name: 'Trail Running', img: 'https://picsum.photos/500/600?random=331' },
              { name: 'Surfing', img: 'https://picsum.photos/500/600?random=332' },
              { name: 'Skiing', img: 'https://picsum.photos/500/600?random=333' },
            ].map((cat, i) => (
              <div key={i} className="group cursor-pointer relative aspect-[4/5] overflow-hidden bg-gray-100">
                <Image src={cat.img} alt={cat.name} fill className="object-cover group-hover:scale-105 transition-transform duration-[1200ms]" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                <div className="absolute bottom-6 left-6">
                  <h3 className="text-white font-black text-xl uppercase tracking-tighter">{cat.name}</h3>
                  <span className="text-white/80 text-[12px] font-bold uppercase tracking-widest flex items-center mt-2">Shop Now <ChevronRight className="w-4 h-4 ml-1" /></span>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="px-4 md:px-8 max-w-[1600px] mx-auto mb-20">
          <div className="flex items-baseline justify-between mb-10 border-b border-gray-300 pb-4">
            <h2 className="text-[32px] md:text-[40px] font-black uppercase tracking-tighter text-[#1a1a1a]">Trending Gear</h2>
            <a href="#" className="text-[14px] font-bold uppercase tracking-widest text-[#1a1a1a] hover:underline underline-offset-8 flex items-center">
              View All <ChevronRight className="w-4 h-4 ml-1" />
            </a>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { title: 'Nano Puff Jacket', desc: 'Lightweight warmth for any adventure.', img: 'https://picsum.photos/600/800?random=334', price: '$229' },
              { title: 'Black Hole Duffel', desc: 'Indestructible travel companion.', img: 'https://picsum.photos/600/800?random=335', price: '$149' },
              { title: 'Capilene Base Layer', desc: 'Moisture-wicking performance.', img: 'https://picsum.photos/600/800?random=336', price: '$69' },
            ].map((item, i) => (
              <div key={i} className="group cursor-pointer bg-white p-4">
                <div className="relative aspect-[3/4] overflow-hidden mb-4 bg-gray-100">
                  <Image src={item.img} alt={item.title} fill className="object-cover group-hover:scale-105 transition-transform duration-[1200ms]" />
                </div>
                <h3 className="text-[18px] font-black uppercase tracking-tighter mb-1 group-hover:underline">{item.title}</h3>
                <p className="text-[14px] text-gray-600 mb-2">{item.desc}</p>
        ...
        {/* ─── CUSTOMER REVIEWS ─── */}
        <section className="px-4 md:px-8 max-w-[1600px] mx-auto py-20">
          <div className="flex items-baseline justify-between mb-10 border-b border-gray-300 pb-4">
            <h2 className="text-[32px] md:text-[40px] font-black uppercase tracking-tighter text-[#1a1a1a]">Community Reviews</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { name: 'Jake R.', text: 'Took my Nano Puff to Patagonia. Survived 40mph winds and freezing rain. Incredible.', rating: 5, location: 'Colorado' },
              { name: 'Sarah K.', text: 'The Worn Wear program is amazing. Got a like-new jacket for half the price.', rating: 5, location: 'Oregon' },
              { name: 'Mike D.', text: 'Best base layer I have ever owned. Keeps me dry on 20-mile trail runs.', rating: 5, location: 'Vermont' },
            ].map((review, i) => (
              <div key={i} className="bg-white p-8 border-l-4 border-[#a2b399]">
                <div className="flex mb-3">
                  {Array.from({ length: review.rating }).map((_, j) => (
                    <span key={j} className="text-orange-500 text-sm">★</span>
                  ))}
                </div>
                <p className="text-[15px] text-gray-700 mb-6 leading-relaxed italic">&ldquo;{review.text}&rdquo;</p>
                <div className="flex items-center justify-between">
                  <p className="text-[13px] font-black uppercase tracking-widest">{review.name}</p>
                  <p className="text-[11px] text-gray-400 font-bold uppercase tracking-widest">{review.location}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="bg-[#1a1a1a] text-white py-20">
          <div className="max-w-[1200px] mx-auto px-6 flex flex-col md:flex-row items-center gap-12">
            <div className="w-full md:w-1/2">
              <Mountain className="w-12 h-12 text-[#a2b399] mb-6" />
              <h2 className="text-[36px] font-black uppercase tracking-tighter mb-4">The EcoOutdoor App</h2>
              <p className="text-[16px] text-gray-400 mb-8 leading-relaxed">
                Find nearby trails, check gear reviews, and shop with exclusive app-only deals. Your outdoor companion.
              </p>
              <div className="flex gap-4">
                <button className="bg-white text-black px-8 py-3 font-black uppercase tracking-widest text-[12px] hover:bg-gray-200 transition-colors">App Store</button>
                <button className="bg-white text-black px-8 py-3 font-black uppercase tracking-widest text-[12px] hover:bg-gray-200 transition-colors">Google Play</button>
              </div>
            </div>
            <div className="w-full md:w-1/2 flex justify-center">
              <div className="w-48 h-80 bg-gray-800 rounded-[2rem] border-4 border-gray-700 shadow-2xl flex items-center justify-center">
                <Mountain className="w-16 h-16 text-white opacity-20" />
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white py-16 border-b border-gray-200">
          <div className="max-w-[1600px] mx-auto px-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
              {[
                { icon: '🏔️', title: 'Ironclad Guarantee', desc: 'If it breaks, we fix or replace it' },
                { icon: '🚚', title: 'Free Shipping', desc: 'On orders over $99' },
                { icon: '♻️', title: 'Worn Wear', desc: 'Trade in used gear for credit' },
                { icon: '🌍', title: '1% for the Planet', desc: 'Every purchase supports the earth' },
              ].map((service, i) => (
                <div key={i} className="flex flex-col items-center p-6">
                  <div className="text-3xl mb-4">{service.icon}</div>
                  <h3 className="text-[13px] font-black uppercase tracking-widest mb-2">{service.title}</h3>
