  ...
  return (
    <div className="min-h-full bg-white font-sans text-[#191919] overflow-x-hidden">
        ...
        <div className="w-full max-w-[1440px] mx-auto px-4 lg:px-8 h-[34px] flex justify-between items-center">
          ...
          <div className="flex items-center space-x-3 lg:space-x-4">
            ...
            >
              ...
              {itemCount > 0 && (
                <span className="absolute -top-2 -right-2 bg-[#dd1e31] text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{itemCount}</span>
              )}
            </div>
          </div>
        </div>
      ...
      <main className="w-full max-w-[1440px] mx-auto px-4 lg:px-8 py-2 md:py-6">
        ...
        <div className="mb-14">
          <div className="flex items-center space-x-4 mb-5">
            <h3 className="text-[20px] font-bold">Daily Deals</h3>
            <span className="text-[13px] text-[#555]">Free shipping included</span>
            <a href="#" className="text-[#0654ba] hover:underline text-[14px] ml-auto">See all</a>
          </div>
          <div className="flex overflow-x-auto gap-4 pb-4 snap-x snap-mandatory hide-scrollbar -mx-4 px-4">
            {data.products.slice(0, 8).map((product, idx) => (
              <div key={`bz-daily-${idx}`} data-product-id={product.id} className="group cursor-pointer shrink-0 w-[200px] snap-start">
                <div className="bg-[#f7f7f7] aspect-square mb-2 rounded-[8px] overflow-hidden relative flex items-center justify-center">
                  <img src={product.imageUrl} alt={product.title} className="max-w-[80%] max-h-[80%] object-contain mix-blend-multiply group-hover:scale-105 transition-transform duration-300" />
                  {product.discount && product.discount > 0 && (
                    <div className="absolute top-2 left-2 bg-[#dd1e31] text-white text-[11px] font-bold px-2 py-0.5 rounded-sm">{product.discount}% OFF</div>
                  )}
                </div>
                <h4 className="text-[13px] line-clamp-1 group-hover:underline">{product.title}</h4>
                ...
                <span className="text-[11px] text-[#86b817] font-bold">Free shipping</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Trending on BidZone */}
        <div className="mb-14 bg-[#f0f6ff] rounded-[16px] p-8">
          <h3 className="text-[20px] font-bold mb-6">Trending on BidZone</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { title: 'Sneakers', img: 'https://picsum.photos/300/300?random=191', count: '2.4M+ items' },
              { title: 'Trading Cards', img: 'https://picsum.photos/300/300?random=192', count: '1.8M+ items' },
              { title: 'Watches', img: 'https://picsum.photos/300/300?random=193', count: '890K+ items' },
              { title: 'Handbags', img: 'https://picsum.photos/300/300?random=194', count: '1.2M+ items' },
            ].map((trend, i) => (
              <div key={i} className="bg-white rounded-[12px] overflow-hidden cursor-pointer group hover:shadow-md transition-shadow">
                <div className="aspect-[4/3] overflow-hidden">
                  <img src={trend.img} alt={trend.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                </div>
                <div className="p-4">
                  <h4 className="font-bold text-[15px] mb-1">{trend.title}</h4>
                  <span className="text-[12px] text-[#555]">{trend.count}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Top Picks For You */}
        <div className="mb-14">
          <h3 className="text-[20px] font-bold mb-5">Top Picks For You</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {data.products.slice(8, 14).map((product, idx) => (
              <div key={`bz-picks-${idx}`} data-product-id={product.id} className="group cursor-pointer flex flex-col">
                <div className="bg-[#f7f7f7] aspect-square mb-2 rounded-[8px] overflow-hidden flex items-center justify-center relative">
                  <img src={product.imageUrl} alt={product.title} className="max-w-[80%] max-h-[80%] object-contain mix-blend-multiply group-hover:scale-105 transition-transform duration-300" />
                </div>
                <h4 className="text-[13px] line-clamp-2 group-hover:underline mb-1">{product.title}</h4>
                <span className="text-[16px] font-bold">${product.price}</span>
                <span className="text-[11px] text-[#555]">Free shipping</span>
              </div>
            ))}
          </div>
        </div>

        {/* Sell on BidZone Banner */}
        <div className="mb-14 border border-gray-200 rounded-[16px] p-8 md:p-10 flex flex-col md:flex-row items-center justify-between bg-white">
          <div className="mb-6 md:mb-0 max-w-[500px]">
            <h3 className="text-[28px] font-bold mb-3">Sell on BidZone</h3>
            <p className="text-[16px] text-[#555] mb-4">Turn your unused items into cash. It's easy to list and sell on BidZone.</p>
            <button className="bg-[#3665f3] text-white px-8 py-3 rounded-full font-bold text-[14px] hover:bg-[#2b50c5] transition-colors">List an item</button>
          </div>
          <div className="grid grid-cols-3 gap-6 text-center">
            {[
              { value: '1.7B+', label: 'Listings' },
              { value: '132M+', label: 'Active buyers' },
              { value: '190+', label: 'Markets' },
            ].map((stat, i) => (
              <div key={i}>
                <div className="text-[24px] font-bold text-[#0064d2]">{stat.value}</div>
                <div className="text-[12px] text-[#555]">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-x-4 gap-y-8 mb-8">
          {paginatedItems.map((product, idx) => (
            <div key={`bidzone-${idx}-${product.id}`} data-product-id={product.id} data-discount={product.discount || 0} className="group cursor-pointer flex flex-col">
              <div className="bg-[#f7f7f7] aspect-square mb-2 flex items-center justify-center overflow-hidden rounded-[8px] relative z-0">
                ...
                {/* Top Rated Seller - usar product.discount como indicador */}
                {product.discount && product.discount > 0 && (
              ...
              <div className="flex flex-col mt-auto pt-1">
                ...
                {product.discount && product.discount > 0 && (
                  <div className="text-[11px] text-[#555] flex items-center mb-0.5">
                    {product.originalPrice && <span className="line-through mr-1.5">${product.originalPrice}</span>}
                    <span className="text-[#191919] font-bold">{product.discount}% OFF</span>
                  </div>
                )}
