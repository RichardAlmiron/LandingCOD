  ...
  return (
    <div className="min-h-full bg-white font-sans text-[#111] overflow-x-hidden">
      ...
      <main className="w-full pb-16">
        ...
        <div className="mt-20 md:mt-24 w-full max-w-[1920px] mx-auto px-4 md:px-12">
          <div className="bg-[#f5f5f5] rounded-none p-10 md:p-16 text-center">
            <h2 className="text-[32px] md:text-[48px] font-black uppercase tracking-tighter leading-none mb-4">BoldAthlete Membership</h2>
            <p className="text-[16px] md:text-[18px] text-[#707070] mb-8 max-w-[600px] mx-auto">Become a Member for the best products, inspiration and stories in sport. <a href="#" className="text-black underline">Learn more</a></p>
            <div className="flex justify-center gap-4">
              <button className="bg-[#111] text-white px-8 py-3 rounded-full font-medium hover:bg-gray-800 transition-colors text-[15px]">Join Us</button>
              <button className="border border-[#ccc] text-[#111] px-8 py-3 rounded-full font-medium hover:border-black transition-colors text-[15px]">Sign In</button>
            </div>
          </div>
        </div>

        {/* ─── SPORT SPOTLIGHT (4 Cards) ─── */}
        <div className="mt-16 md:mt-24 w-full max-w-[1920px] mx-auto px-4 md:px-12">
          <h2 className="text-[24px] font-medium mb-6">Sport Spotlight</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            {[
              { name: 'Running', img: 'https://picsum.photos/500/600?random=47' },
              { name: 'Basketball', img: 'https://picsum.photos/500/600?random=48' },
              { name: 'Training', img: 'https://picsum.photos/500/600?random=49' },
              { name: 'Lifestyle', img: 'https://picsum.photos/500/600?random=50' },
            ].map((sport, i) => (
              <div key={i} className="relative aspect-[3/4] group cursor-pointer overflow-hidden bg-[#f5f5f5]">
                <Image src={sport.img} alt={sport.name} fill className="object-cover group-hover:scale-[1.03] transition-transform duration-500" referrerPolicy="no-referrer" />
                <div className="absolute bottom-6 left-6">
                  <h3 className="text-white text-[20px] font-medium mb-3 drop-shadow-md">{sport.name}</h3>
                  <button className="bg-white text-black px-5 py-2 rounded-full text-[13px] font-medium hover:bg-gray-200 transition-colors">Shop</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* ─── ESSENTIALS PICKS ─── */}
        <div className="mt-16 md:mt-24 w-full max-w-[1920px] mx-auto px-4 md:px-12">
          <h2 className="text-[24px] font-medium mb-6">The Essentials</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
            {[
              { name: "Men's", img: 'https://picsum.photos/700/500?random=51' },
              { name: "Women's", img: 'https://picsum.photos/700/500?random=52' },
              { name: "Kids'", img: 'https://picsum.photos/700/500?random=53' },
            ].map((ess, i) => (
              <div key={i} className="relative aspect-[4/3] group cursor-pointer overflow-hidden bg-[#f5f5f5]">
                <Image src={ess.img} alt={ess.name} fill className="object-cover group-hover:scale-[1.02] transition-transform duration-500" referrerPolicy="no-referrer" />
                <div className="absolute bottom-6 left-6">
                  <button className="bg-white text-black px-5 py-2 rounded-full text-[13px] font-medium hover:bg-gray-200 transition-colors">{ess.name}</button>
                </div>
              </div>
            ))}
          </div>
