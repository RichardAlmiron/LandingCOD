...
import { calculateDiscount, calculateDiscountedPrice } from '@/lib/utils';
  ...
  return (
    <div className="min-h-full bg-[#f5f5f5] font-sans text-[#222] overflow-x-hidden">
          ...
          <div className="flex items-center space-x-2 shrink-0">
            ...
            >
              <div className="w-10 h-10 flex items-center justify-center relative">
                ...
                <span className="absolute top-0 right-0 bg-[#fd384f] text-white text-[12px] font-bold px-[6px] py-[1px] rounded-full border-2 border-white leading-none">{itemCount}</span>
              </div>
      ...
      <main className="w-full max-w-[1500px] mx-auto px-2 md:px-4 xl:px-8 py-4">
        ...
        <div className="bg-white rounded-[16px] p-4 sm:p-6 mb-6 shadow-sm">
          ...
          <div className="flex overflow-x-auto space-x-4 pb-2 snap-x [&::-webkit-scrollbar]:hidden md:custom-scrollbar">
            {flashDeals.map((product, i) => (
              <div key={i} className="shrink-0 w-[140px] sm:w-[180px] bg-white group cursor-pointer" data-product-id={product.id} data-discount={product.discount || 0}>
                ...
                <div className="flex items-baseline mb-1">
                  <span className="text-[12px] font-bold text-[#fd384f] align-top mr-0.5">{product.currency || 'Gs.'}</span>
                  <span className="text-[20px] font-bold text-[#fd384f] leading-none">
                    {product.discount && product.discount > 0 
                ...
                <div className="text-[12px] text-gray-400 line-through mb-1">
                  {product.currency || 'Gs.'}{Math.floor(product.price).toLocaleString('es-PY')}
                </div>
                {/* Product discount if set */}
                {product.discount && product.discount > 0 && (
                  <div className="text-[12px] text-[#cc0c39] font-bold mb-1">{product.discount}% OFF</div>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
            {paginatedItems.map((product, idx) => (
              <div key={`flashdeals-${idx}-${product.id}`} data-product-id={product.id} data-discount={product.discount || 0} className="bg-white rounded-[16px] overflow-hidden hover:shadow-[0_4px_12px_rgba(0,0,0,0.1)] transition-all cursor-pointer group flex flex-col h-full border border-gray-100 hover:border-transparent">
                {/* Product Image */}
                  ...
                  <div className="mt-auto">
                    ...
                    <div className="flex items-baseline mb-0.5">
                      <span className="text-[18px] font-bold text-[#222]">
                        {product.currency || 'Gs.'} {product.discount && product.discount > 0 
                          ? calculateDiscountedPrice(product.price, product.discount).toLocaleString('es-PY')
                          : product.price.toLocaleString('es-PY')}
                      </span>
                    </div>
                    {/* Real Discount Display */}
                    {product.discount && product.discount > 0 ? (
                      <div className="flex items-center gap-2 mb-1.5">
                        <span className="text-[12px] text-gray-400 line-through">{product.currency || 'Gs.'} {product.price.toLocaleString('es-PY')}</span>
                        <span className="text-[11px] font-bold text-[#fd384f]">-{product.discount}%</span>
                      </div>
                    ) : null}
        ...
        {/* ─── CHOICE PICKS - Curated Selection ─── */}
        <div className="bg-white rounded-[16px] p-4 sm:p-6 mb-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              <span className="bg-[#1b1b1b] text-white text-[13px] font-black italic px-3 py-1 rounded-tl-[8px] rounded-br-[8px] rounded-tr-[2px] rounded-bl-[2px]">Choice</span>
              <h3 className="text-[18px] font-bold text-[#222]">Selección Curada</h3>
            </div>
            <a href="#" className="text-[14px] text-gray-500 hover:text-[#fd384f] font-medium flex items-center">
              Ver más <ChevronRight className="w-4 h-4" />
            </a>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {data.products.slice(0, 4).map((p, i) => (
              <div key={`choice-${i}`} data-product-id={p.id} className="bg-[#f5f5f5] rounded-[12px] p-3 cursor-pointer group hover:shadow-md transition-all">
                <div className="aspect-square rounded-[8px] overflow-hidden bg-white mb-2 flex items-center justify-center p-2">
                  <img src={p.imageUrl} alt={p.title} className="max-h-full max-w-full object-contain mix-blend-multiply group-hover:scale-105 transition-transform" />
                </div>
                <h4 className="text-[13px] text-[#222] line-clamp-1 font-medium mb-1">{p.title}</h4>
                <div className="flex items-baseline gap-1">
                  <span className="text-[16px] font-bold text-[#222]">{p.currency || 'Gs.'} {Math.floor(p.price).toLocaleString('es-PY')}</span>
                </div>
                <div className="flex items-center gap-1 mt-1">
                  <Star className="w-[10px] h-[10px] fill-current text-[#222]" />
                  <span className="text-[11px] font-bold text-[#222]">{p.rating || 4.5}</span>
                  <span className="text-[11px] text-[#999]">{p.reviews || 100}+ vendidos</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* ─── COUPON CENTER - Promo Banner ─── */}
        <div className="bg-gradient-to-r from-[#fd384f] to-[#ff6b6b] rounded-[16px] p-6 md:p-8 mb-6 shadow-sm cursor-pointer group overflow-hidden relative">
          <div className="absolute -right-10 -top-10 w-40 h-40 border-[30px] border-white/10 rounded-full"></div>
          <div className="absolute right-20 -bottom-10 w-24 h-24 border-[15px] border-white/10 rounded-full"></div>
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="text-white text-center md:text-left">
              <div className="text-[12px] font-bold uppercase tracking-widest mb-1 opacity-80">Centro de Cupones</div>
              <h3 className="text-[28px] md:text-[36px] font-black leading-tight">Hasta 70% OFF</h3>
              <p className="text-[14px] text-white/80 mt-1">Cupones exclusivos para nuevos usuarios. Válidos por 72h.</p>
            </div>
            <button className="bg-white text-[#fd384f] font-bold text-[14px] px-8 py-3 rounded-full hover:bg-gray-100 transition-colors shadow-lg group-hover:scale-105 transform duration-200">
              Obtener Cupones
            </button>
          </div>
        </div>

        {/* ─── CATEGORIES VISUAL - Circular Icons ─── */}
        <div className="bg-white rounded-[16px] p-4 sm:p-6 mb-6 shadow-sm">
          <h3 className="text-[18px] font-bold text-[#222] mb-5">Explorar Categorías</h3>
          <div className="grid grid-cols-4 md:grid-cols-8 gap-4">
            {['Electrónica', 'Moda', 'Hogar', 'Belleza', 'Deportes', 'Juguetes', 'Joyería', 'Motor'].map((cat, i) => (
              <div key={`cat-${i}`} className="flex flex-col items-center cursor-pointer group">
                <div className="w-[60px] h-[60px] md:w-[72px] md:h-[72px] rounded-full bg-[#f5f5f5] flex items-center justify-center mb-2 group-hover:bg-[#fff0f0] group-hover:ring-2 group-hover:ring-[#fd384f]/20 transition-all overflow-hidden p-2">
                ...
                <span className="text-[11px] md:text-[12px] font-medium text-[#222] text-center group-hover:text-[#fd384f] transition-colors">{cat}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-[16px] p-4 sm:p-6 mb-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              <h3 className="text-[18px] font-bold text-[#222]">Zona Nuevos Usuarios</h3>
              <span className="bg-[#fff0f0] text-[#fd384f] text-[11px] font-bold px-2 py-0.5 rounded-full">Desde $0.99</span>
            </div>
          </div>
          <div className="flex overflow-x-auto space-x-3 pb-2 [&::-webkit-scrollbar]:hidden">
            {data.products.slice(2, 8).map((p, i) => (
              <div key={`newuser-${i}`} data-product-id={p.id} className="shrink-0 w-[130px] cursor-pointer group">
                <div className="aspect-square rounded-[12px] overflow-hidden bg-[#f5f5f5] mb-2 flex items-center justify-center p-2 relative">
                  <img src={p.imageUrl} alt={p.title} className="max-h-full max-w-full object-contain mix-blend-multiply group-hover:scale-105 transition-transform" />
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-[#fd384f] to-transparent h-[30px] flex items-end justify-center pb-1">
                    <span className="text-[10px] text-white font-bold">Oferta especial</span>
                  </div>
                </div>
                <div className="text-[14px] font-bold text-[#fd384f]">{p.currency || 'Gs.'} {Math.floor(p.price * 0.1).toLocaleString('es-PY')}</div>
                <div className="text-[11px] text-gray-400 line-through">{p.currency || 'Gs.'} {Math.floor(p.price).toLocaleString('es-PY')}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* ─── APP DOWNLOAD BANNER ─── */}
        <div className="bg-[#1b1b1b] rounded-[16px] p-6 md:p-8 mb-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            <div className="w-16 h-16 bg-[#fd384f] rounded-2xl flex items-center justify-center shadow-lg shrink-0">
              <Smartphone className="w-8 h-8 text-white" />
            </div>
            <div className="text-white">
              <h3 className="text-[20px] font-bold">Descarga la App</h3>
              <p className="text-[14px] text-gray-400 mt-1">10% de descuento extra en tu primer pedido desde la app</p>
            </div>
          </div>
          <div className="flex gap-3">
            <button className="bg-white text-[#222] font-bold text-[13px] px-6 py-3 rounded-full hover:bg-gray-100 transition-colors">
              App Store
            </button>
            <button className="bg-white text-[#222] font-bold text-[13px] px-6 py-3 rounded-full hover:bg-gray-100 transition-colors">
              Google Play
        ...
        <div className="bg-white rounded-[16px] p-4 sm:p-6 mb-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[18px] font-bold text-[#222]">Mejor Valorados</h3>
            <a href="#" className="text-[13px] text-[#fd384f] font-bold hover:underline">Ver todos</a>
          </div>
          <div className="flex overflow-x-auto space-x-4 pb-2 [&::-webkit-scrollbar]:hidden">
            {data.products.slice(0, 6).map((p, i) => (
              <div key={`toprated-${i}`} className="shrink-0 w-[200px] bg-[#fafafa] rounded-xl p-3 cursor-pointer group hover:shadow-md transition-shadow">
                ...
                <div className="flex items-center gap-1 mb-1">
                  ...
                  <span className="text-[12px] font-bold text-[#222]">{p.rating || 4.5}</span>
                  <span className="text-[11px] text-gray-400">({p.reviews || 100})</span>
                </div>
                <h4 className="text-[12px] text-[#222] line-clamp-1 font-medium">{p.title}</h4>
                <span className="text-[14px] font-bold text-[#fd384f]">{p.currency || 'Gs.'} {Math.floor(p.price).toLocaleString('es-PY')}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-[16px] p-4 sm:p-6 mb-6 shadow-sm">
          <h3 className="text-[18px] font-bold text-[#222] mb-4">Visto Recientemente</h3>
          <div className="flex overflow-x-auto space-x-3 pb-2 [&::-webkit-scrollbar]:hidden">
            {data.products.slice(2, 8).map((p, i) => (
              <div key={`recent-fd-${i}`} className="shrink-0 w-[130px] cursor-pointer group">
                <div className="h-[130px] bg-[#f7f7f7] rounded-xl flex items-center justify-center p-2 mb-1">
                  <img src={p.imageUrl} alt={p.title} className="max-h-full object-contain mix-blend-multiply group-hover:scale-105 transition-transform" />
                </div>
                <span className="text-[13px] font-bold text-[#fd384f]">{p.currency || 'Gs.'} {Math.floor(p.price).toLocaleString('es-PY')}</span>
              </div>
            ))}
          </div>
        </div>
