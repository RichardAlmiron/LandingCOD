  ...
  return (
    <div className="min-h-full bg-white font-sans text-[#222] overflow-x-hidden" style={{ fontFamily: "'RedStyle Sans Regular', 'Helvetica Neue', Helvetica, Arial, sans-serif" }}>
          ...
          <div className="w-full flex justify-between items-center md:items-start absolute top-4 px-4 md:px-6 z-10">
            ...
            <div className="flex items-center space-x-4 md:space-x-5 text-[12px] font-bold">
              ...
              >
                ...
                <span className="hidden md:block font-normal text-[11px]">Shopping bag ({itemCount})</span>
                <span className="md:hidden absolute top-[-4px] right-[-4px] bg-[#e50010] text-white text-[9px] w-4 h-4 flex items-center justify-center rounded-full font-bold">{itemCount}</span>
              </div>
            </div>
          </div>
      ...
      <main className="w-full mx-auto px-4 md:px-0">
        ...
        <div className="w-full max-w-[1440px] mx-auto mb-20 md:mb-32 px-0 md:px-6">
          <h2 className="text-[20px] md:text-[26px] font-bold text-center mb-10 tracking-tight">Get Inspired</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {[303, 304, 305, 306, 307, 308, 309, 310].map((num) => (
              <div key={num} className="relative aspect-square group cursor-pointer overflow-hidden">
                <Image src={`https://picsum.photos/500/500?random=${num}`} alt="Style" fill className="object-cover group-hover:scale-110 transition-transform duration-700" />
          ...
          <div className="text-center mt-6">
            <a href="#" className="text-[13px] font-bold hover:underline">@RedStyle · Follow us on Instagram</a>
          </div>
        </div>

        {/* ─── SERVICES BAR ─── */}
        <div className="w-full bg-white border-y border-gray-200 py-12 mb-20">
          <div className="max-w-[1440px] mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-8 text-center">
            {[
              { icon: '🚚', title: 'Free Shipping', desc: 'On orders over $40 for members' },
              { icon: '↩️', title: 'Free Returns', desc: 'Easy returns within 30 days' },
              { icon: '🎁', title: 'Member Rewards', desc: 'Earn points on every purchase' },
              { icon: '♻️', title: 'Garment Collecting', desc: 'Bring old clothes, get a voucher' },
            ].map((svc, i) => (
              <div key={i} className="flex flex-col items-center">
                <span className="text-3xl mb-3">{svc.icon}</span>
                <h4 className="text-[14px] font-bold mb-1">{svc.title}</h4>
                <p className="text-[13px] text-[#666]">{svc.desc}</p>
              </div>
            ))}
          </div>
        </div>
