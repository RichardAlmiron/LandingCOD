            ...
            <div className="w-full flex items-center justify-between gap-12">
              ...
              <div className="flex items-center gap-10 text-stone-800">
                ...
                >
                  ...
                  {itemCount > 0 && (
                    <span className="absolute -top-1 -right-2 bg-stone-950 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full ring-2 ring-white">{itemCount}</span>
                  )}
                </div>
                <button 
      ...
      <main className="max-w-[1536px] mx-auto px-8 py-16">
        ...
        <div className="mb-40">
          <div className="text-center mb-20">
            <div className="text-[11px] font-black tracking-[0.4em] text-stone-300 uppercase mb-4">Explore</div>
            <h2 className="text-5xl font-serif italic tracking-tight text-stone-950">Shop by Category</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8">
            {[
              { name: 'Women', img: 'https://picsum.photos/300/400?random=480' },
              { name: 'Men', img: 'https://picsum.photos/300/400?random=481' },
              { name: 'Kids', img: 'https://picsum.photos/300/400?random=482' },
              { name: 'Home', img: 'https://picsum.photos/300/400?random=483' },
              { name: 'Beauty', img: 'https://picsum.photos/300/400?random=484' },
              { name: 'Designer', img: 'https://picsum.photos/300/400?random=485' },
            ].map((cat, i) => (
              <div key={i} className="group cursor-pointer text-center">
                <div className="relative aspect-square overflow-hidden rounded-full mb-4 border-2 border-stone-100 group-hover:border-stone-950 transition-all">
                  <img src={cat.img} alt={cat.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                </div>
                <h4 className="font-black text-[11px] uppercase tracking-[0.2em] group-hover:text-stone-500 transition-colors">{cat.name}</h4>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mb-40">
          <div className="flex items-end justify-between mb-16 border-b border-stone-200 pb-8">
            <div>
              <div className="text-[11px] font-black tracking-[0.4em] text-stone-300 uppercase mb-2">Staff Favorites</div>
              <h2 className="text-4xl font-serif italic tracking-tight text-stone-950">Trending This Season</h2>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-10">
            {data.products.slice(0, 4).map((product, i) => (
              <div key={`pick-${i}`} className="group cursor-pointer">
                <div className="relative aspect-[3/4] overflow-hidden bg-stone-50 rounded-sm mb-6">
                  ...
                  <button onClick={(e) => handleAddToCart(product, e)} className="absolute bottom-4 left-4 right-4 bg-stone-950 text-white py-3 font-black uppercase text-[11px] tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">Add to Bag</button>
                </div>
                <h3 className="text-[13px] font-medium line-clamp-1 mb-1">{product.title}</h3>
                <span className="text-lg font-bold tracking-tighter">${product.price}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Customer Reviews */}
        <div className="mb-40 bg-stone-50 rounded-lg p-12 lg:p-20 border border-stone-100">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-serif italic tracking-tight text-stone-950">What Our Clients Say</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {[
              { name: 'Emily S.', text: 'The Nordy Club benefits are incredible. Free alterations saved me hundreds this year.', stars: 5 },
              { name: 'Robert K.', text: 'Best customer service in retail. The personal stylists are truly exceptional.', stars: 5 },
              { name: 'Amanda W.', text: 'Love the Anniversary Sale. I plan my entire wardrobe around it every year.', stars: 5 },
            ].map((review, i) => (
              <div key={i} className="text-center">
                <div className="flex justify-center mb-4">{Array.from({ length: review.stars }).map((_, j) => <Star key={j} className="w-4 h-4 fill-stone-950 text-stone-950" />)}</div>
                <p className="text-sm font-light text-stone-600 mb-6 leading-relaxed italic">"{review.text}"</p>
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-stone-950">{review.name}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Value Props Strip */}
        <div className="mb-40 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            { icon: <Truck className="w-6 h-6" />, title: 'Free Shipping & Returns', desc: 'Always, no minimum' },
            { icon: <Scissors className="w-6 h-6" />, title: 'Expert Alterations', desc: 'Perfect fit guaranteed' },
            { icon: <Gift className="w-6 h-6" />, title: 'Gift Services', desc: 'Complimentary wrapping' },
            { icon: <ShieldCheck className="w-6 h-6" />, title: 'Price Match', desc: 'We match competitor prices' },
          ].map((val, i) => (
            <div key={i} className="flex flex-col items-center text-center p-8 border border-stone-100 rounded-lg hover:border-stone-950 transition-colors group">
              <div className="text-stone-400 group-hover:text-stone-950 transition-colors mb-4">{val.icon}</div>
              <h4 className="font-black text-[11px] uppercase tracking-[0.2em] mb-2">{val.title}</h4>
              <p className="text-[11px] text-stone-400 font-medium">{val.desc}</p>
            </div>
          ))}
        </div>
        ...
        {/* App & Newsletter */}
        <div className="mb-40 bg-stone-100 rounded-3xl p-12 lg:p-20 flex flex-col md:flex-row items-center justify-between border border-stone-200">
          <div className="mb-8 md:mb-0 max-w-lg">
            <div className="flex items-center gap-3 mb-4">
              <Smartphone className="w-8 h-8 text-stone-950" />
              <h2 className="text-3xl font-serif italic text-stone-950">Get the LuxService App</h2>
            </div>
            <p className="text-stone-500 font-light leading-relaxed">Shop smarter with personalized recommendations, scan & shop in store, and manage your Nordy Club rewards on the go.</p>
          </div>
          <div className="flex gap-4">
            <button className="bg-stone-950 text-white px-8 py-4 font-black uppercase text-[11px] tracking-widest hover:bg-stone-800 transition-colors rounded-lg shadow-xl">App Store</button>
            <button className="bg-stone-950 text-white px-8 py-4 font-black uppercase text-[11px] tracking-widest hover:bg-stone-800 transition-colors rounded-lg shadow-xl">Google Play</button>
          </div>
        </div>
