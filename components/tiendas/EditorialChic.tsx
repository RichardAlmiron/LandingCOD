    ...
    <div className="min-h-full bg-white font-sans text-black overflow-x-hidden selection:bg-black selection:text-white" style={{ fontFamily: "'Neue Helvetica', Helvetica, Arial, sans-serif" }}>
        ...
        <div className="w-full px-4 md:px-10 py-6 md:py-8 flex items-start justify-between">
          ...
          <div className="flex items-start space-x-6 md:space-x-10 text-[11px] md:text-[13px] uppercase tracking-widest font-medium pointer-events-auto">
            ...
            >
              ...
              {itemCount > 0 && (
                <span className="absolute top-2 right-[-8px] text-[9px] font-bold">{itemCount}</span>
              )}
            </div>
          </div>
        </div>
      ...
      <main className="w-full">
        ...
        <div className="w-full flex flex-col pt-20 md:pt-32">
          ...
          {/* Lookbook Strip (3 editorial images) */}
          <div className="w-full px-4 md:px-8 mb-32 md:mb-48">
            <h2 className="text-[12px] md:text-[14px] uppercase tracking-[0.15em] font-medium mb-8 px-2 md:px-4">Lookbook / SR26</h2>
            <div className="grid grid-cols-3 gap-2 md:gap-4">
              {[13, 14, 15].map((n) => (
                <div key={n} className="relative aspect-[2/3] overflow-hidden group cursor-pointer bg-gray-100">
                  <Image src={`https://picsum.photos/600/900?random=${n}`} alt={`Look ${n}`} fill className="object-cover group-hover:scale-105 transition-transform duration-[1500ms]" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="w-full relative h-[50vh] md:h-[70vh] mb-32 md:mb-48 overflow-hidden group cursor-pointer bg-black">
            <Image src="https://picsum.photos/1920/1080?random=21" alt="Campaign" fill className="object-cover opacity-70 group-hover:scale-105 transition-transform duration-[2000ms]" />
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-white text-center">
                <p className="text-[11px] md:text-[13px] uppercase tracking-[0.3em] mb-4">Campaign Film</p>
                <h2 className="text-[28px] md:text-[48px] font-serif tracking-[-0.03em]">The New Silhouette</h2>
              </div>
            </div>
          </div>

          {/* Sustainability Section */}
          <div className="w-full px-6 md:px-24 mb-32 md:mb-48">
            <div className="border-t border-black pt-12 flex flex-col md:flex-row gap-12">
              <div className="md:w-1/2">
                <h2 className="text-[11px] md:text-[13px] uppercase tracking-[0.15em] font-medium mb-6">Join Life</h2>
                <p className="text-[14px] md:text-[16px] text-gray-600 leading-relaxed max-w-md">Our commitment to caring for the planet. Discover our sustainable collections made with organic cotton, recycled materials, and responsible processes.</p>
              </div>
              <div className="md:w-1/2 relative aspect-[4/3] overflow-hidden bg-gray-100">
                <Image src="https://picsum.photos/800/600?random=22" alt="Sustainability" fill className="object-cover" />
              </div>
            </div>
          </div>
