  ...
  return (
    <div className="min-h-full bg-[#f4f2eb] font-sans text-[#1a1a1a] overflow-x-hidden selection:bg-[#1b3c35] selection:text-[#f4f2eb]">
        ...
        <div className="w-full mx-auto px-4 md:px-8 h-[60px] md:h-[80px] flex items-center justify-between">
          ...
          <div className="flex items-center justify-end space-x-6 w-1/3">
            ...
            >
              ...
              {itemCount > 0 && (
                <span className="absolute -top-2 -right-2 bg-[#1b3c35] text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{itemCount}</span>
              )}
            </button>
          </div>
        </div>
      ...
      <main className="w-full">
        ...
        {/* ─── NEWSLETTER ─── */}
        <div className="w-full bg-[#1b3c35] text-[#f4f2eb] py-20 px-6">
          <div className="max-w-[600px] mx-auto text-center">
            <h3 className="text-[24px] md:text-[32px] font-serif mb-4">Stay Connected</h3>
            <p className="text-[14px] font-sans font-light opacity-80 mb-8">Subscribe to receive the latest news, collections, and exclusive invitations from ItalianCraft.</p>
            <form className="flex flex-col sm:flex-row gap-3">
              <input type="email" placeholder="Email Address" className="flex-1 bg-transparent border-b border-[#f4f2eb]/30 py-3 text-[12px] font-sans tracking-wide outline-none focus:border-[#f4f2eb] transition-colors placeholder-[#f4f2eb]/50" />
              <button type="button" className="border border-[#f4f2eb] px-8 py-3 text-[11px] uppercase tracking-[0.2em] hover:bg-[#f4f2eb] hover:text-[#1b3c35] transition-colors">Subscribe</button>
            </form>
          </div>
        </div>

        {/* ─── GIFT GUIDE ─── */}
        <div className="w-full max-w-[1920px] mx-auto px-4 md:px-12 py-24 bg-[#f4f2eb]">
          <div className="text-center mb-16">
            <h3 className="text-[28px] md:text-[36px] font-serif mb-4">The Art of Gifting</h3>
            <p className="text-[13px] font-sans font-light text-gray-500">Find the perfect gift for every occasion</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {[
              { title: 'For Her', img: 'https://picsum.photos/400/500?random=275' },
              { title: 'For Him', img: 'https://picsum.photos/400/500?random=276' },
              { title: 'Wedding', img: 'https://picsum.photos/400/500?random=277' },
              { title: 'Baby', img: 'https://picsum.photos/400/500?random=278' },
            ].map((gift, i) => (
              <div key={i} className="group cursor-pointer">
                <div className="relative aspect-[4/5] overflow-hidden mb-4">
                  <Image src={gift.img} alt={gift.title} fill className="object-cover group-hover:scale-105 transition-transform duration-[1500ms]" />
                </div>
                <h4 className="text-center text-[13px] font-serif group-hover:underline underline-offset-4">{gift.title}</h4>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* ─── STORE LOCATOR ─── */}
        <div className="relative h-[50vh] md:h-[60vh] w-full overflow-hidden flex items-center justify-center group cursor-pointer">
          <Image src="https://picsum.photos/1600/900?random=279" alt="Store" fill className="object-cover group-hover:scale-[1.03] transition-transform duration-[3000ms]" />
          <div className="absolute inset-0 bg-black/30" />
          <div className="relative z-10 text-center text-white p-10 bg-white/10 backdrop-blur-md border border-white/20 max-w-[500px] w-[90%]">
            <MapPin className="w-8 h-8 mx-auto mb-6 stroke-[1]" />
            <h3 className="text-[28px] font-serif mb-4">Find a Boutique</h3>
            <p className="text-[13px] font-sans font-light mb-8 opacity-90">Discover ItalianCraft boutiques near you.</p>
            <button className="bg-white text-[#1a1a1a] px-10 py-3.5 text-[11px] uppercase tracking-[0.2em] font-medium hover:bg-[#1b3c35] hover:text-white transition-colors">Store Locator</button>
          </div>
        </div>
