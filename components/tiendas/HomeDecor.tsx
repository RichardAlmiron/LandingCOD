  ...
  return (
    <div className="min-h-full bg-white font-sans text-[#222222] selection:bg-[#7f187f] selection:text-white overflow-x-hidden pb-10">
          ...
          <div className="flex items-center space-x-3 md:space-x-6 shrink-0 text-gray-700">
            ...
            >
              <div className="relative">
                ...
                {itemCount > 0 && (
                  <span className="absolute -top-1.5 -right-2 bg-[#7f187f] text-white text-[11px] font-bold w-5 h-5 flex items-center justify-center rounded-full leading-none border-2 border-white shadow-sm">{itemCount}</span>
                )}
              </div>
      ...
      <main className="w-full max-w-[1440px] mx-auto px-4 lg:px-8 py-6 md:py-10">
        ...
        <div className="mb-16">
          <h2 className="text-[24px] md:text-[28px] font-black mb-6 text-[#222]">Room Inspiration</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { room: 'Living Room', img: 'https://picsum.photos/600/400?random=160', items: '2,340 items' },
              { room: 'Bedroom', img: 'https://picsum.photos/600/400?random=161', items: '1,890 items' },
              { room: 'Kitchen', img: 'https://picsum.photos/600/400?random=162', items: '1,560 items' },
            ].map((r, i) => (
              <div key={i} className="group cursor-pointer relative overflow-hidden rounded-[8px] aspect-[3/2]">
                <Image src={r.img} alt={r.room} fill className="object-cover group-hover:scale-105 transition-transform duration-700" referrerPolicy="no-referrer" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                <div className="absolute bottom-4 left-4 text-white">
                  <h3 className="text-[20px] font-bold mb-1">{r.room}</h3>
                  <span className="text-[13px] opacity-80">{r.items}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mb-16 relative overflow-hidden rounded-[12px] h-[280px] md:h-[350px] group cursor-pointer">
          <Image src="https://picsum.photos/1400/400?random=163" alt="Trending" fill className="object-cover group-hover:scale-[1.02] transition-transform duration-1000" referrerPolicy="no-referrer" />
          <div className="absolute inset-0 bg-gradient-to-r from-[#7f187f]/80 to-transparent" />
          <div className="absolute inset-y-0 left-0 flex flex-col justify-center p-8 md:p-12 text-white max-w-lg">
            <span className="text-[12px] font-bold uppercase tracking-widest mb-2 opacity-80">Trending Now</span>
            <h2 className="text-[28px] md:text-[36px] font-black mb-3 leading-tight">Modern Farmhouse Style</h2>
            <p className="text-[15px] mb-6 opacity-90">Rustic charm meets contemporary comfort. Shop the look.</p>
            <button className="bg-white text-[#7f187f] px-6 py-3 rounded-full font-bold text-[14px] hover:bg-gray-100 transition-colors w-fit">Shop the Trend</button>
          </div>
        </div>

        {/* ─── BRAND SPOTLIGHT ─── */}
        <div className="mb-16">
          <h2 className="text-[24px] md:text-[28px] font-black mb-6 text-[#222]">Brand Spotlight</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { name: 'Joss & Main', tagline: 'Stylish finds for every room', img: 'https://picsum.photos/300/300?random=164' },
              { name: 'AllModern', tagline: 'Modern design made simple', img: 'https://picsum.photos/300/300?random=165' },
              { name: 'Birch Lane', tagline: 'Classic home furnishings', img: 'https://picsum.photos/300/300?random=166' },
              { name: 'Perigold', tagline: 'Luxury home décor', img: 'https://picsum.photos/300/300?random=167' },
            ].map((brand, i) => (
              <div key={i} className="bg-white border border-gray-200 rounded-[8px] overflow-hidden group cursor-pointer hover:shadow-lg transition-shadow">
                <div className="relative aspect-square">
                  <Image src={brand.img} alt={brand.name} fill className="object-cover group-hover:scale-105 transition-transform duration-500" referrerPolicy="no-referrer" />
                </div>
                <div className="p-4 text-center">
                  <h3 className="font-bold text-[16px] text-[#222] mb-1">{brand.name}</h3>
                  <p className="text-[13px] text-gray-500">{brand.tagline}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* ─── RECENTLY VIEWED ─── */}
        <div className="mb-16">
          <h2 className="text-[24px] md:text-[28px] font-black mb-6 text-[#222]">Recently Viewed</h2>
          <div className="flex overflow-x-auto gap-4 pb-4 snap-x hide-scrollbar">
            {data.products.slice(0, 6).map((product, idx) => (
              <div key={`rv-${idx}`} className="shrink-0 w-[200px] snap-start group cursor-pointer">
                <div className="relative aspect-square mb-3 bg-gray-50 rounded-[6px] overflow-hidden">
                  <Image src={product.imageUrl} alt={product.title} fill className="object-contain p-2 group-hover:scale-105 transition-transform duration-500" referrerPolicy="no-referrer" />
                </div>
                <h3 className="text-[13px] font-medium line-clamp-1 group-hover:underline">{product.title}</h3>
                <span className="text-[15px] font-black text-[#222]">${product.price}</span>
              </div>
            ))}
          </div>
        </div>
