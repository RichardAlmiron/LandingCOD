  ...
  return (
    <div className="min-h-full bg-white font-sans text-[#333] overflow-x-hidden">
          ...
          <div className="flex items-center space-x-1 lg:space-x-3 2xl:space-x-6 shrink-0 h-full">
            ...
            >
              <div className="relative">
                ...
                {itemCount > 0 && (
                  <span className="absolute -top-1.5 -right-2 bg-[#cc0000] text-white text-[11px] font-bold w-[18px] h-[18px] flex items-center justify-center rounded-full border-2 border-white">{itemCount}</span>
                )}
              </div>
            </button>
          </div>
      ...
      <main className="w-full max-w-[1440px] mx-auto px-4 lg:px-8 py-6">
        ...
        {/* Trending Now */}
        <div className="mb-14">
          <h2 className="text-[22px] font-bold mb-6 tracking-tight">Trending Now</h2>
          <div className="flex overflow-x-auto gap-4 pb-4 snap-x snap-mandatory hide-scrollbar -mx-4 px-4">
            {data.products.slice(6, 12).map((product, idx) => (
              <div key={`bs-trend-${idx}`} data-product-id={product.id} className="group cursor-pointer shrink-0 w-[200px] snap-start">
                <div className="relative aspect-square bg-[#f4f4f4] rounded-[8px] overflow-hidden mb-3">
                  <img src={product.imageUrl} alt={product.title} className="w-full h-full object-contain p-4 mix-blend-multiply group-hover:scale-105 transition-transform duration-300" />
                </div>
                <h3 className="text-[13px] line-clamp-2 hover:underline mb-1">{product.title}</h3>
        ...
        {/* RedCard Banner */}
        <div className="mb-14 bg-[#cc0000] rounded-[16px] p-8 md:p-10 text-white flex flex-col md:flex-row items-center justify-between">
          <div className="mb-6 md:mb-0">
            <h3 className="text-[28px] font-black mb-2">Save 5% every day</h3>
            <p className="text-[16px]">with your Bullseye RedCard. Plus free shipping on most items.</p>
          </div>
          <button className="bg-white text-[#cc0000] px-8 py-3 rounded-full font-bold text-[15px] hover:bg-gray-100 transition-colors">Apply now</button>
        </div>

        {/* New Arrivals Grid */}
        <div className="mb-14">
          <h2 className="text-[22px] font-bold mb-6 tracking-tight">New Arrivals</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {data.products.slice(12, 16).map((product, idx) => (
              <div key={`bs-new-${idx}`} data-product-id={product.id} className="group cursor-pointer rounded-[8px] hover:shadow-md transition-shadow p-2">
                <div className="relative aspect-square bg-[#f4f4f4] rounded-[8px] overflow-hidden mb-3">
                  <img src={product.imageUrl} alt={product.title} className="w-full h-full object-contain p-4 mix-blend-multiply group-hover:scale-105 transition-transform duration-300" />
                  <div className="absolute top-2 left-2 bg-[#006601] text-white text-[11px] font-bold px-2 py-0.5 rounded-[4px]">New</div>
                </div>
                <h3 className="text-[14px] line-clamp-2 hover:underline mb-1">{product.title}</h3>
        ...
        <div className="mb-14">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 lg:gap-4">
            {paginatedItems.map((product, i) => (
              <div key={product.id} data-product-id={product.id} data-discount={product.discount || 0} className="group cursor-pointer flex flex-col h-full rounded-[8px] hover:shadow-[0_4px_12px_rgba(0,0,0,0.1)] transition-shadow p-2 -m-2">
                <div className="relative aspect-square mb-3 bg-[#f4f4f4] rounded-[8px] overflow-hidden">
                  <img src={product.imageUrl} alt={product.title} className="w-full h-full object-contain mix-blend-multiply p-4 group-hover:scale-105 transition-transform duration-300" />
                ...
                <div className="flex-1 flex flex-col">
                  ...
                  <div className="flex items-center space-x-1 mb-3 mt-auto">
                    ...
                    <span className="text-[11px] text-gray-600">({product.reviews || 0})</span>
                  </div>
                </div>
              </div>
            ))}
