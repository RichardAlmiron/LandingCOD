  ...
  return (
    <div className="min-h-full bg-[#f8f9fa] font-mono text-slate-900 selection:bg-[#ff6600] selection:text-white antialiased">
      ...
      <main className="max-w-full mx-auto p-8 lg:p-12">
        ...
        {/* Trending Builds Showcase */}
        </section>
        <section className="mb-24">
          <div className="flex items-end justify-between mb-12 border-b-4 border-[#003366] pb-8">
            <div className="space-y-3">
              <h2 className="text-5xl font-black uppercase tracking-tighter text-[#003366] flex items-center gap-4">
                <Star className="w-10 h-10 text-[#ff6600]" /> TRENDING BUILDS
              </h2>
              <p className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.4em]">Community-voted configurations this week</p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { name: 'Ultra Gaming Rig', specs: 'RTX 5090 / i9-14900K / 64GB DDR5', price: '$4,299', img: 'https://picsum.photos/600/400?random=500' },
              { name: 'Content Creator Pro', specs: 'RTX 4080 / Ryzen 9 7950X / 128GB', price: '$3,599', img: 'https://picsum.photos/600/400?random=501' },
              { name: 'Budget Beast', specs: 'RTX 4060 Ti / i5-14600K / 32GB', price: '$1,299', img: 'https://picsum.photos/600/400?random=502' },
            ].map((build, i) => (
              <div key={i} className="bg-white border-2 border-slate-200 rounded-2xl overflow-hidden hover:border-[#ff6600] transition-all group cursor-pointer">
                <div className="relative h-[250px] overflow-hidden">
                  <img src={build.img} alt={build.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                  <div className="absolute top-4 left-4 bg-[#ff6600] text-white text-[9px] font-black uppercase px-3 py-1.5 rounded flex items-center gap-1">
                    <Star className="w-3 h-3 fill-white" /> TOP RATED
                  </div>
                </div>
                <div className="p-8 space-y-4">
                  <h3 className="text-xl font-black uppercase tracking-tight text-[#003366]">{build.name}</h3>
                  <p className="text-slate-400 font-bold text-[11px] uppercase tracking-widest">{build.specs}</p>
                  <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                    <span className="text-3xl font-black text-[#003366]">{build.price}</span>
                    <button className="bg-[#003366] text-white px-6 py-3 rounded font-black uppercase text-[10px] tracking-widest hover:bg-[#ff6600] transition-colors">View Build</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        {/* Deal of the Day */}
        <section className="mb-24 bg-gradient-to-r from-[#003366] to-[#001a33] rounded-[2rem] p-12 lg:p-20 flex flex-col lg:flex-row items-center gap-12 text-white overflow-hidden relative">
          <div className="absolute inset-0 opacity-[0.05] bg-[url('https://www.transparenttextures.com/patterns/circuit-board.png')] pointer-events-none" />
          <div className="flex-1 space-y-8 relative z-10">
            <div className="flex items-center gap-3">
              <div className="bg-[#ff6600] px-4 py-2 rounded font-black text-[10px] uppercase tracking-widest flex items-center gap-2">
                <Clock className="w-4 h-4" /> DEAL OF THE DAY
              </div>
              <span className="text-white/40 font-black text-[10px] uppercase tracking-widest">ENDS IN 08:42:15</span>
            </div>
            <h2 className="text-5xl font-black uppercase tracking-tighter leading-none">CORSAIR VENGEANCE <span className="text-[#ff6600]">DDR5-6000</span></h2>
            <p className="text-blue-100/60 text-lg font-medium max-w-xl">64GB (2x32GB) high-performance memory kit with Intel XMP 3.0 support. Perfect for next-gen gaming and content creation workloads.</p>
            <div className="flex items-baseline gap-4">
              <span className="text-5xl font-black text-[#ff6600]">$189.99</span>
              <span className="text-2xl text-white/30 line-through font-bold">$249.99</span>
              <span className="bg-red-600 text-white text-[10px] font-black px-3 py-1 rounded uppercase">Save 24%</span>
            </div>
            <button className="bg-[#ff6600] text-white px-16 py-5 rounded font-black uppercase text-lg hover:bg-white hover:text-[#003366] transition-all shadow-2xl">
              Grab This Deal
            </button>
          </div>
          <div className="flex-1 relative h-[350px] hidden lg:block">
            <img src="https://picsum.photos/600/400?random=505" alt="Deal" className="w-full h-full object-contain drop-shadow-2xl" />
          </div>
        </section>

        {/* Tech Reviews / Community */}
        <section className="mb-24">
          <div className="flex items-end justify-between mb-12 border-b-4 border-[#003366] pb-8">
            <div className="space-y-3">
              <h2 className="text-5xl font-black uppercase tracking-tighter text-[#003366]">COMMUNITY REVIEWS</h2>
              <p className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.4em]">What builders are saying about their latest upgrades</p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { user: 'BuildMaster_X', rating: 5, text: 'The RTX 5090 is an absolute beast. 4K gaming at 120fps is now a reality.', product: 'NVIDIA RTX 5090' },
              { user: 'TechEnthusiast', rating: 5, text: 'DDR5-8000 made a noticeable difference in my rendering workflows. Highly recommend.', product: 'G.Skill Trident Z5' },
              { user: 'PCBuilder2026', rating: 4, text: 'Solid motherboard with great VRM thermals. Perfect for overclocking the 14900K.', product: 'ASUS ROG Maximus' },
              { user: 'GamerPro99', rating: 5, text: 'This NVMe drive is insanely fast. Load times are practically non-existent now.', product: 'Samsung 990 Pro' },
            ].map((review, i) => (
              <div key={i} className="bg-white border-2 border-slate-100 rounded-xl p-8 hover:border-[#ff6600] transition-colors space-y-4">
                <div className="flex items-center gap-1">
                  {Array.from({ length: review.rating }).map((_, s) => (
                    <Star key={s} className="w-4 h-4 fill-[#ff6600] text-[#ff6600]" />
                  ))}
                </div>
                <p className="text-slate-600 text-sm font-medium leading-relaxed">"{review.text}"</p>
                <div className="pt-4 border-t border-slate-100">
                  <p className="font-black text-[10px] uppercase tracking-widest text-[#003366]">{review.user}</p>
                  <p className="text-slate-400 text-[9px] font-bold uppercase tracking-widest mt-1">{review.product}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="mb-24 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { icon: ShieldCheck, title: 'Buyer Protection', desc: 'Every purchase is backed by our comprehensive buyer protection program.' },
            { icon: Truck, title: 'Express Shipping', desc: 'Free 2-day shipping on orders over $99. Same-day available in select areas.' },
            { icon: HardDrive, title: '30-Day Returns', desc: 'Not satisfied? Return any unopened item within 30 days for a full refund.' },
            { icon: Wrench, title: 'Tech Support', desc: '24/7 expert technical support for all your hardware questions and issues.' },
          ].map((item, i) => (
            <div key={i} className="bg-white border-2 border-slate-100 rounded-2xl p-8 text-center hover:border-[#ff6600] transition-colors group">
              <div className="w-16 h-16 bg-[#003366] rounded-xl flex items-center justify-center mx-auto mb-6 group-hover:bg-[#ff6600] transition-colors">
                <item.icon className="w-8 h-8 text-white" />
              </div>
              <h3 className="font-black uppercase text-sm tracking-widest text-[#003366] mb-3">{item.title}</h3>
              <p className="text-slate-400 text-sm font-medium">{item.desc}</p>
            </div>
          ))}
        </section>

        {/* Mobile Lab + Shell Shocker */}
        <section className="py-32 grid grid-cols-1 lg:grid-cols-2 gap-12">
