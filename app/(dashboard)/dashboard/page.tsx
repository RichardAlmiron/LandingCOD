...
function Sparkline({ color, index }: { color: string; index: number }) {
    // Deterministic data per index to avoid hydration mismatch (no Math.random)
    const seeds = [
        [18, 32, 14, 38, 22, 40, 28, 35],
        [25, 15, 30, 20, 36, 12, 34, 26],
        [10, 28, 22, 38, 16, 34, 30, 20],
        [30, 14, 36, 18, 32, 24, 12, 38],
    ];
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
                ...
                {stats.map((s, i) => (
                    <div key={s.label} className="ultra-glass glass-card-hover" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 12, cursor: 'pointer', position: 'relative', overflow: 'hidden' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative', zIndex: 1 }}>
                            ...
                            <Sparkline color={s.color} index={i} />
                        </div>
                        {/* Decorative glow */}
            ...
            <div className="ultra-glass" style={{ padding: '0 0 4px 0', overflow: 'hidden', position: 'relative' }}>
                <div className="hyper-ring" style={{ opacity: 0.1 }}></div>
                <div style={{ padding: '12px 20px 8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.05)', position: 'relative', zIndex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 800, color: '#fff' }}>Proyectos Recientes</div>
